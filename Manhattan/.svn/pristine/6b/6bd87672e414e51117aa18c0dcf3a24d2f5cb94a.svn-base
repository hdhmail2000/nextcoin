//
//  ChZScanQRController.m
//  FuturePurse
//
//  Created by Howe on 2018/8/2.
...
@interface ChZScanQRController ()<AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
/// 会话对象
...
@implementation ChZScanQRController
...
//    self.toChZConfigHandle = YES;
    ...
    //    [self.view addSubview:self.scanningView];
    [self _setupScanningView];
    [self setupSGQRCodeScanning];
    if (self.type == ScanTypeAdd)
    {
        self.nameLabel.text = @"扫一扫";
    }
    if (self.type == ScanTypePay)
    {
        self.nameLabel.text = @"付款";
    }
}
...
- (void)readImageFromAlbum {
    ...
    if (device) {
        ...
        if (status == PHAuthorizationStatusNotDetermined) { // 用户还没有做出选择
            ...
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"⚠️ 警告" message:@"请去-> [设置 - 隐私 - 照片 - 通链] 打开访问开关" preferredStyle:(UIAlertControllerStyleAlert)];
