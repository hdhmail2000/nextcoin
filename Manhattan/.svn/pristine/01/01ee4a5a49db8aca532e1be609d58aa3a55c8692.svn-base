...
#pragma mark - - - 扫描边角imageView的创建
    // 左上侧的image
    CGFloat margin = 7;
    CGFloat imageViewX = CGRectGetMinX(scanContent_layer.frame)  + margin;
    CGFloat imageViewY = CGRectGetMinY(scanContent_layer.frame)  + margin;
    UIImageView *imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"SGQRCode.bundle/public_qrcode_bg"]];
    imageView.frame = CGRectMake(imageViewX, imageViewY, scanContent_layerW - margin * 2, scanContent_layerH - margin * 2);
    [self.tempLayer addSublayer:imageView.layer];
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.image = [UIImage imageNamed:@"SGQRCode.bundle/public_qrcode_line"];
