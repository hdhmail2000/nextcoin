...
-(void)requestKineData{
    @weakify(self);
    [ChZHomeRequest requestC2CKineLineCoinNmae:self.coinName
                                          time:self.requestDate
...
-(void)hanleKineData:(NSArray *)array{
    ...
    if(array)
    {
        ...
        for(XHHC2CKineModel * model in array){
            ...
            [dates addObject:[ChZUtil chz_getDateWithTimeIntervalSince1970:model.inputtime]];
        }
...
- (ZFWaveChart *)waveChart
{
    if(!_waveChart)
    {
        ...
        _waveChart.isShowYLineSeparate = YES;
        _waveChart.separateLineStyle = kLineStyleDashLine;
        _waveChart.separateColor = [UIColor colorWithHexString:@"4B4B80"];
        _waveChart.axisLineNameColor = [UIColor colorWithHexString:@"4B4B80"];
