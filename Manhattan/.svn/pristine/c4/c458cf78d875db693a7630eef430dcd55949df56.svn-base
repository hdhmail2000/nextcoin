...
-(instancetype)initWithFrame:(CGRect)frame lineColor:(UIColor *)color Titles:(NSArray <NSString *>*)titles viewControllers:(NSArray <UIViewController *>*)viewControllers
{
   XLSlideSwitch *obj = [self initWithFrame:frame Titles:titles viewControllers:viewControllers];
    
    UIView *bottomLine = [UIView new];
    bottomLine.backgroundColor = [UIColor colorWithHexString:@"363672"];
    [_segment addSubview:bottomLine];
//    _segment.bottomLine = bottomLine;
    return obj;
}
...
- (void)buildUI {
    ...
    _segment.frame = CGRectMake(8, 0, self.bounds.size.width, SegmentHeight);
    _segment.delegate = self;
    [self addSubview:_segment];
    
    UIView *bottomLine = [UIView new];
    bottomLine.backgroundColor = [UIColor colorWithHexString:@"363672"];
    bottomLine.frame = CGRectMake(0, SegmentHeight-1, self.bounds.size.width, 2);
    [self addSubview:bottomLine];
    [self bringSubviewToFront:_segment];
...
-(void)showInViewController:(UIViewController *)viewController isScroll:(BOOL)isScroll
{
    if (isScroll)[viewController addChildViewController:_pageVC];
