...
-(void)clickMiningArea:(NSNotification *)noti
{
    ...
    if (model)
    {
        ...
        {
            [self showTheAlertVCWithStyle:UIAlertControllerStyleAlert title:nil message:@"您暂未开通此矿区，是否立即开通？" title1:@"取消" action1:^{
        ...
        {
            ChZ_MBError(@"您的矿区还未产生币");
            return;
        }
        [self requestMingGetId:model];
    }
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)requestMingGetId:(XHHMiningHomeModel *)miningModel{
    @weakify(self);
    [XHHMiningRequest requestMiningInComeGetId:miningModel.getid success:^(id dataSource)
     {
        @strongify(self);
        [self requestHomeData];
        [self requestMineList:YES];
        [self handleAudio];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"GETMININGSUCCESS" object:miningModel];
    } error:^(int code, NSString *error) {
        ChZ_DebugLog(@"%@--",error);
        if (![error containsString:@"已领取"])
        {
            ChZ_MBError(error);
        }
