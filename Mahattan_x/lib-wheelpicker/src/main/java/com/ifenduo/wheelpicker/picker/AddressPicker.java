package com.ifenduo.wheelpicker.picker;
...
import com.ifenduo.wheelpicker.util.LogUtils;
import com.ifenduo.wheelpicker.widget.WheelView;
...
public class AddressPicker extends LinkagePicker<Province, City, County> {
    ...
    public County getSelectedCounty() {
        if(getSelectedCity()!=null &&getSelectedCity().getCounties()!=null && getSelectedCity().getCounties().size()>0){
    ...
    private static class AddressProvider implements Provider<Province, City, County> {
        ...
        public List<County> linkageThirdData(int firstIndex, int secondIndex) {
            if (thirdList != null && thirdList.size() > firstIndex && thirdList.get(firstIndex) != null && thirdList.get(firstIndex).size() > secondIndex) {
