...
public class KLineGridAxisDrawing implements IDrawing {
    ...
    public void onInit(RectF contentRect, AbstractRender render) {
        ...
        if (yLabelAlign == YLabelAlign.RIGHT_IN || yLabelAlign == YLabelAlign.LEFT_OUT) {
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            ...
            if (i % count == 0) {
                ...
                canvas.drawText(
                        entrySet.getEntryList().get(i).getxLabel_(),
                        pointCache[0],
    ...
    public void onDrawOver(Canvas canvas) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            float labelX = 0;
            if (yLabelAlign == YLabelAlign.LEFT_IN) {
                labelX = kLineRect.left + 5;
            } else if (yLabelAlign == YLabelAlign.RIGHT_IN) {
                labelX = kLineRect.right - 5;
            } else if (yLabelAlign == YLabelAlign.RIGHT_OUT) {
                labelX = kLineRect.right + 5;
            } else {
                labelX = kLineRect.left - 5;
            }

//            float labelX = yLabelAlign == YLabelAlign.LEFT_IN ? kLineRect.left + 5 : kLineRect.right - 5;
