...
//        if (!((buyEntrust.getFuid() == 10719 && sellEntrust.getFuid() == 10723)
//                || (buyEntrust.getFuid() == 10723 && sellEntrust.getFuid() == 10719))){
//            // 买价格 < 卖价格
...
//		}
        ...
        // 更新用户钱包
        UserCoinWallet buyWallet = this.userCoinWalletMapper.selectLock(buyEntrust.getFuid(), buyEntrust.getFbuycoinid());
        ...
        if (buyEntrust.getFuid().equals(sellEntrust.getFuid())) {
            ...
            buyWallet.setTotal(total);
            buyWallet.setFrozen(frozen);
            buyWallet.setGmtModified(Utils.getTimestamp());
            buyWalletLog += "Afrozen:" + buyWallet.getFrozen() + ",vol:" + successAmount + "],";
            sellWalletLog += "Atotal:" + buyWallet.getTotal() + ",vol:" + totalTmp + ",fee:" + sellAmountFee + "],";
            if (this.userCoinWalletMapper.update(buyWallet) <= 0) {
                throw new Exception("buyWallet update err");
            }
        } else {
            sellWallet = this.userCoinWalletMapper.selectLock(sellEntrust.getFuid(), sellEntrust.getFbuycoinid());
            buyWalletLog += "{RMB:[Bfrozen:" + buyWallet.getFrozen() + ",";
            ...
            sellWallet.setTotal(total);
            sellWallet.setGmtModified(Utils.getTimestamp());
            buyWalletLog += "Afrozen:" + buyWallet.getFrozen() + ",vol:" + successAmount + "],";
            sellWalletLog += "Atotal:" + sellWallet.getTotal() + ",vol:" + totalTmp + ",fee:" + sellAmountFee + "],";
            if (this.userCoinWalletMapper.update(buyWallet) <= 0) {
                throw new Exception("buyWallet update err");
            }
            if (this.userCoinWalletMapper.update(sellWallet) <= 0) {
                throw new Exception("sellWallet update err");
            }
        }
        // 更新用户虚拟币钱包
        UserCoinWallet buyVirtualWallet = this.userCoinWalletMapper.selectLock(buyEntrust.getFuid(), buyEntrust.getFsellcoinid());
        ...
        if (buyEntrust.getFuid().equals(sellEntrust.getFuid())) {
            ...
            buyVirtualWallet.setTotal(total);
            buyVirtualWallet.setFrozen(frozen);
            buyVirtualWallet.setGmtModified(Utils.getTimestamp());
            buyWalletLog += "Atotal:" + buyVirtualWallet.getTotal() + ",vol:" + totalTmp + ",fee:" + buyCountFee + "],my:1}";
            sellWalletLog += "Afrozen:" + buyVirtualWallet.getFrozen() + ",vol:" + successCount + "],my:1}";
            if (this.userCoinWalletMapper.update(buyVirtualWallet) <= 0) {
                throw new Exception();
            }
        } else {
            sellVirtualWallet = this.userCoinWalletMapper.selectLock(sellEntrust.getFuid(), sellEntrust.getFsellcoinid());
            BigDecimal totalTmp = MathUtils.sub(successCount, buyCountFee);
            BigDecimal total = MathUtils.add(buyVirtualWallet.getTotal(), totalTmp);
            buyWalletLog += "Coin:[Btotal:" + buyVirtualWallet.getTotal() + ",";
            ...
            sellVirtualWallet.setFrozen(frozen);
            sellVirtualWallet.setGmtModified(Utils.getTimestamp());
            buyWalletLog += "Atotal:" + buyVirtualWallet.getTotal() + ",vol:" + totalTmp + ",fee:" + buyCountFee + "]}";
            sellWalletLog += "Afrozen:" + sellVirtualWallet.getFrozen() + ",vol:" + successCount + "]}";
            if (this.userCoinWalletMapper.update(buyVirtualWallet) <= 0) {
                throw new Exception("buyVirtualWallet update err");
            }
            if (this.userCoinWalletMapper.update(sellVirtualWallet) <= 0) {
                throw new Exception("sellVirtualWallet update err");
            }
        }

        //卖的佣金
        //commissionFee(sellAmountFee,sellEntrust);
        //买的佣金
        ...
        if (buyEntrust.getFstatus().equals(EntrustStateEnum.AllDeal.getCode())) {
            ...
            if (leftAmount.compareTo(BigDecimal.ZERO) > 0) {
                BigDecimal totalRmb = MathUtils.add(buyWallet.getTotal(), leftAmount);
                BigDecimal frozenRmb = MathUtils.sub(buyWallet.getFrozen(), leftAmount);
                buyWallet.setFrozen(frozenRmb);
                buyWallet.setTotal(totalRmb);
                buyWallet.setGmtModified(Utils.getTimestamp());
                if (this.userCoinWalletMapper.update(buyWallet) <= 0) {
    ...
    public synchronized void updateCancelMatch(FEntrust fEntrust) throws Exception {
        ...
        if (fEntrust.getFtype().equals(EntrustTypeEnum.BUY.getCode())) {
            // 查询钱包
            UserCoinWallet fUserWallet = userCoinWalletMapper.selectLock(fEntrust.getFuid(), fEntrust.getFbuycoinid());
            if (fUserWallet == null) {
            ...
            // 更新钱包
            fUserWallet.setTotal(walletTotal);
            fUserWallet.setFrozen(walletFrozen);
            fUserWallet.setGmtModified(Utils.getTimestamp());
            if (this.userCoinWalletMapper.update(fUserWallet) <= 0) {
                throw new Exception("userWallet update err");
            }
        } else if (fEntrust.getFtype().equals(EntrustTypeEnum.SELL.getCode())) {
            // 查询钱包
            UserCoinWallet fUserVirtualWallet = userCoinWalletMapper.selectLock(fEntrust.getFuid(), fEntrust.getFsellcoinid());
            ...
            // 更新钱包
            fUserVirtualWallet.setTotal(walletTotal);
            fUserVirtualWallet.setFrozen(walletFrozen);
            fUserVirtualWallet.setGmtModified(Utils.getTimestamp());
            if (this.userCoinWalletMapper.update(fUserVirtualWallet) <= 0) {
