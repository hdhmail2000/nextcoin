...
//        va.setAccessKey("N8490311");
//        va.setSecretKey("K0rjtXY8c");
//        va.setUrl("http://smssh1.253.com/msg/send/json");
//
//        ValidateSmsDO vs = new ValidateSmsDO();
//        vs.setContent("【VCBTC】验证码 2014 请勿泄露！");
//        vs.setPhone("15064555948");
//        send253NormalSMS(va,vs);

//        ValidateAccountDO va = new ValidateAccountDO();
//        va.setAccessKey("LTAI1vBsrDdQddRl");
//        va.setSecretKey("sZFjDuQsKZlmz4UvL7IwsSPd9xmCrc");
//        va.setUrl("ccnex@push.ccnex.com");
//
//        ValidateEmailDO ve = new ValidateEmailDO();
//        ve.setEmail("yangxuefengy@gmail.com");
//        ve.setTitle("ccnex");
//        ve.setContent("测试");
//
//        sendAliEmail(va,ve);
    ...
    public Boolean updateSend(ValidateSendDTO vs) {
        ...
        if (vs.getSendType().equals(SendTypeEnum.EMAIL.getCode()) &&
                vs.getBusinessType().equals(BusinessTypeEnum.FINANCE_BALANCE.getCode())) {
            return mailDrawBitsConsume(account, vs.getEmail(), vs.getCustomUrl(), vs.getUsername());
        }
        ...
        if (template == null && vs.getBusinessType() != BusinessTypeEnum.SMS_WEB_REGISTER.getCode()) {
        ...
        // 组装发送请求
        if (vs.getSendType().equals(SendTypeEnum.EMAIL.getCode())) {
            return mailMsgConsume(vs, template, platform, content, account);
        } else {
...
//            }
            ...
            return smsMsgConsume(vs, template, content, account);
        }
    }
    private Boolean smsMsgConsume(ValidateSendDTO vs, ValidateTemplateDO template, String content,
                                  ValidateAccountDO account) {
    ...
    private Boolean mailDrawBitsConsume(ValidateAccountDO account, String mailAddress, String content, String title) {
    ...
    private Boolean mailMsgConsume(ValidateSendDTO vs, ValidateTemplateDO template, ValidatePlatformDO platform,
                                   String content, ValidateAccountDO account) {
    ...
    private boolean send253NormalSMS(ValidateAccountDO va,ValidateSmsDO vs) {
        ...
        logger.info("account = "+va.getAccessKey()+"password = "+va.getSecretKey()+"msg = "+vs.getContent()+"phone = "+vs.getPhone()+"url = "+va.getUrl());
        String submitString = jsonObject.toString();
        logger.info("submitString = "+submitString);
...
//            System.out.println("msg = "+vs.getContent());
//            System.out.println("phone = "+vs.getPhone());
//            System.out.println("code = "+jsonObj.getString("code"));
//            System.out.println("errorMsg = "+jsonObj.getString("errorMsg"));
            logger.info("msg = "+vs.getContent()+"phone = "+vs.getPhone()+"code = "+jsonObj.getString("code")+"errorMsg = "+jsonObj.getString("errorMsg"));
...
//    }
    ...
    private static boolean sendAliEmail(ValidateAccountDO va, ValidateEmailDO ve) {
        ...
        try {
            ...
            logger.error("sendAliEmail failed, info:" + JSON.toJSONString(va) + JSON.toJSONString(ve), e);
            flag = false;
        }
        return flag;
    }
