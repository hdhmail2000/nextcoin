...
public class ValidateHelper {
    ...
    public Boolean mailDrawBits(String email, PlatformEnum platform, String customUrl, String title) {
        if (StringUtils.isEmpty(email) || StringUtils.isEmpty(customUrl) || StringUtils.isEmpty(title)
                || platform == null ) {
            return false;
        }
        String uuid = GUIDUtils.getGUIDString();
        ValidateSendDTO sendDTO = new ValidateSendDTO();
        sendDTO.setBusinessType(BusinessTypeEnum.FINANCE_BALANCE.getCode());
        sendDTO.setEmail(email);
        sendDTO.setPlatformType(platform.getCode());
        sendDTO.setSendType(SendTypeEnum.EMAIL.getCode());
        sendDTO.setUsername(title);
        sendDTO.setCustomUrl(customUrl);
        return mailSendBase(sendDTO);
    }
