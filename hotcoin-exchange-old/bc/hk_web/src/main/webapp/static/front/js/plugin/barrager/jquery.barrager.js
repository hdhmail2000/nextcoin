(function($) {
	$.fn.barrager = function(barrage) {
		...
		div_barrager_box.append("<a class='portrait z' href='javascript:;'><i class='icon z'></i></a>");
		if (barrage.img) {
		...
		div_barrager.css("width", ((div_barrager.outerWidth()+10)) + "px");
		div_barrager.css("right", (-(div_barrager.outerWidth()+10)) + "px");
		var i = 0;
		div_barrager.css('margin-right', i);
		var looper = setInterval(barrager, barrage.speed);
		function barrager() {
			var window_width = $(window).width() + $(id).outerWidth();
			if (i < window_width) {
				i += 1;
