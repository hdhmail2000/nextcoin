...
MainDataSource.prototype.clearDataItems = function () {
	this._dataItems = [];
};
...
MainDataSource.prototype.getTime = function (date) {
	var etime=date;
	var globalurl = util.globalurl[GLOBAL_VAR.coinshortName];
    ...
    $(document).ready(function(){
        ...
        $("#chart_container .chart_toolbar_tabgroup a")
            .click(function() {
            	...
            	if(settings.charts.period == name){
            		return;
            	}
                switch_period(name);
            });
...
var RequestData = function(showLoading) {
    ...
    $(document).ready(		
        GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
            type        : "get",
            url         : globalurl.kline.format(globalurl.klineType[GLOBAL_VAR.time_type]),
            dataType    : 'text',
            ...
            success     : function(json) {            	
                if (GLOBAL_VAR.G_HTTP_REQUEST) {
                    ...
                    window.parent.market.hideLoading();
                    ChartManager.getInstance().redraw('All', false);                    
                }
            },
...
function getRealData(){
	...
	if(globalurl.isremote){
		...
		url=globalurl.kline.format(globalurl.klineType[GLOBAL_VAR.time_type]);
		var klineCallback = function(data){
		...
		util.network({
			url : url,
			method : "get",
			success : klineCallback,
		});
		realtimeid=window.setTimeout(function() {
			getRealData();
		}, 2000);
	}else{
		url=globalurl.depth+"?"+GLOBAL_VAR.requestParam;		
		var depthCallback = function(data){
			data=data.data;
			var asksdata = new Array();
			var asks = data.depth.asks;
			var length = asks.length-1;
			for(var i = length; i >= 0; i--){
				asksdata.push(asks[i]);
			}
			data.depth.asks = asksdata;
