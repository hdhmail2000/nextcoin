...
if __name__ == '__main__':
    ...
    if nowTime in range(1, 4):
        amount = round(random.uniform(0.1, 0.001), 4)
    elif nowTime in range(6, 8):
        amount = round(random.uniform(0.15, 0.002), 3)
    elif nowTime in range(13, 16):
        amount = round(random.uniform(0.3, 0.001), 3)
    elif nowTime in range(18, 22):
        amount = round(random.uniform(0.2, 0.001), 3)
    else:
        amount = round(random.uniform(0.33, 0.1), 2)

    buyOne = b.buy("eth_cnyt", price, amount)
    if buyOne.get('status') == 'ok':
        sellOne = b.sell("eth_cnyt", price, amount)
