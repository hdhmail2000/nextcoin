...
if __name__ == '__main__':
    ...
    # print accountInfo
    tickers = b.tickers("sc_cnyt")
    print tickers

    price = round(random.uniform(tickers.get('buyOne'), tickers.get('sellOne')), 5)

    nowTime = datetime.datetime.now().hour
    if nowTime in range(1, 5):
        amount = round(random.uniform(100, 1000), 1)
    elif nowTime in range(6, 8):
        amount = round(random.uniform(1000, 2500), 1)
    elif nowTime in range(13, 16):
        amount = round(random.uniform(2000, 3000), 1)
    ...
    if round == 1.0:
        buyOne = b.buy("sc_cnyt", price, amount)
        if buyOne.get('status') == 'ok':
            sellOne = b.sell("sc_cnyt", price, amount)
