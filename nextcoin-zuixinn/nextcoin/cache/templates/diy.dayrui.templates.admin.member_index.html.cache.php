	...
	<form method="post" action="" name="searchform" id="searchform">
		...
		<label>
			<select class="form-control" name="data[groupid]">
				<option value=""> -- </option>
				<?php $return = array();$list_temp = $this->list_tag("action=cache name=MEMBER.group"); if ($list_temp) extract($list_temp); $count=count($return); if (is_array($return)) { foreach ($return as $key=>$t) { ?>
...
<form action="" method="post" name="myform" id="myform">
	...
	<div class="portlet mylistbody">
		...
		<tr>
			...
			<th class="<?php echo ns_sorting('trade_status'); ?>" name="regtime" ><?php echo fc_lang('C2C交易状态'); ?></th>
		...
		<tr id="dr_row_<?php echo $t['uid']; ?>">
			...
			<!--<td><a <?php if ($this->ci->is_auth('member/experience')) { ?>href="<?php echo dr_url('member/experience',array('uid'=>$t['uid'])); ?>" style="color: blue;text-decoration: underline;font-size: 12px;"<?php } ?>><?php echo $t['experience']; ?></a></td>
			<td><a <?php if ($this->ci->is_auth('member/score')) { ?>href="<?php echo dr_url('member/score',array('uid'=>$t['uid'])); ?>" style="color: blue;text-decoration: underline;font-size: 12px;"<?php } ?>><?php echo $t['score']; ?></a></td>
			<td><?php echo $t['money']; ?></td>-->
			<td><?php if ($t['trade_status'] == 1) { ?>可发布<?php } else if ($t['trade_status'] == 0) { ?>不可发布<?php } ?></td>
			<td><?php echo dr_date($t['regtime'], NULL, 'red'); ?></td>
			<td class="dr_option">
			<?php if ($this->ci->is_auth('member/edit')) { ?><a class="aedit" <?php if ($t['id']==1) { ?>href="javascript:;"<?php } else { ?>href="<?php echo dr_url('member/edit',array('uid'=>$t['uid'])); ?>"<?php } ?>> <i class="fa fa-edit"></i> <?php echo fc_lang('修改'); ?></a><?php }  if (MEMBER_OPEN_SPACE && $this->ci->is_auth('space/admin/space/edit')) { ?><a class="alist" href="<?php echo dr_url('space/space/edit',array('uid' => $t['uid'])); ?>"> <i class="fa fa-database"></i> <?php echo fc_lang('空间'); ?></a><?php }  if ($member['adminid']==1) { ?><a class="ago" href="<?php echo dr_member_url('api/ologin'); ?>&uid=<?php echo $t['uid']; ?>" target="_blank"> <i class="fa fa-send"></i> <?php echo fc_lang('登录会员'); ?></a><?php } ?>
			</td>
		</tr> 
		<?php } } ?>
		<tr class="mtable_bottom">
			...
			<td colspan="10"  >
            <?php if ($this->ci->is_auth('member/del')) { ?>
            ...
            <?php }  if ($this->ci->is_auth('member/edit')) { ?>
            ...
            <?php } ?>
            <label><button type="button" class="btn green btn-sm" name="option" onClick="$('#action').val('user2yes');dr_confirm_set_all('<?php echo fc_lang('您确定要这样操作吗？'); ?>')"><i class="fa fa-edit"></i> <?php echo fc_lang('指定可发布交易'); ?></button></label>
            <label><button type="button" class="btn green btn-sm" name="option" onClick="$('#action').val('user2no');dr_confirm_set_all('<?php echo fc_lang('您确定要这样操作吗？'); ?>')"><i class="fa fa-edit"></i> <?php echo fc_lang('指定不可发布交易'); ?></button></label>
            <label><button type="button" class="btn green btn-sm" name="option" onClick="$('#action').val('userAllno');dr_confirm_set_all('<?php echo fc_lang('您确定要这样操作吗？'); ?>')"><i class="fa fa-edit"></i> <?php echo fc_lang('全部用户不可发布交易'); ?></button></label>
            <label><button type="button" class="btn green btn-sm" name="option" onClick="$('#action').val('userAllyes');dr_confirm_set_all('<?php echo fc_lang('您确定要这样操作吗？'); ?>')"><i class="fa fa-edit"></i> <?php echo fc_lang('全部用户可发布交易'); ?></button></label>
			</td>
		</tr>
		</tbody>
		</table>
