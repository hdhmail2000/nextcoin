...
class Member_model extends CI_Model {
    ...
    public function _register($data, $OAuth = NULL, $groupid = NULL, $uid = NULL) {
        ...
        if ($OAuth) {
            ...
            if ($this->ci->get_cache('member', 'setting', 'regoauth')) {
                ...
                !$data['nickname'] && $data['nickname'] = 'oauth';
                $data['username'] = $data['nickname'];
                // 账号重复
                if ($data['nickname'] != 'oauth'
                    && $this->db->where('username', $data['username'])->count_all_results('member')) {
                    $data['username'] = 'oauth';
                }
            } else {
                // 绑定账号
            ...
            if ($regverify == 1) {
                ...
                // 人工审核
                $this->admin_notice('member', fc_lang('新会员【%s】注册审核', $data['username']), 'admin/member/index/field/uid/keyword/'.$uid);
            }
        }
    ...
    public function cache() {
        ...
        if ($cache['setting']['ucenter']) {
            $s = '<?php if (!defined(\'BASEPATH\')) exit(\'No direct script access allowed\');'
                ...
                . '?>';
            file_put_contents(WEBPATH.'api/ucenter/poscms.inc.php', $s, LOCK_EX);
        }
    ...
    public function delete($uids) {
        return $this->member_delete($uids);
    }

    public function member_delete($uids) {
