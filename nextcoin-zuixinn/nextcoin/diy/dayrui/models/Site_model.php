...
class Site_model extends CI_Model {
    ...
    public function cache() {
        ...
        $cache = $domain = $module_domain = $client = array();
        ...
        foreach ($data as $id => $t) {
            ...
            // 终端域名
            if ($t['setting']['client']) {
                foreach ($t['setting']['client'] as $ii => $a) {
                    if ($a) {
                        $domain[$a] = $id;
                        $client[$a] = $ii;
                    }
            ...
            foreach ($this->config as $cv => $cn) {
                if (!isset($t['setting'][$cv])) {
                    $t['setting'][$cv] = '';
                }
            }
            // 写入配置文件
            $this->dconfig->file(WEBPATH.'config/site/'.$id.'.php')->note('站点配置文件')->space(32)->to_require_one($this->config, $t['setting'], 1);
            $this->dconfig->file(WEBPATH.'config/client.php')->note('站点终端配置文件')->space(32)->to_require_one($this->config, $t['setting'], 1);
            // 写入缓存文件
            $cache[$id] = $t['setting'];
                ...
                foreach ($site as $sid => $s) {
                    if ($s['use']) {
                        ...
                        if ($s['mobile_domain']) {
                            ...
                            $module_domain[$s['mobile_domain']] = 'm_'.$t['dirname']; // 模块域名归类
                        }
        ...
        $this->dconfig->file(WEBPATH.'config/client_domain.php')->note('站点终端域名文件')->space(32)->to_require_one($client);
