...
class Linkage_model extends CI_Model {
	 ...
	 * @return	
	 */
	public function edits($key, $id, $data, $save = array()) {
        ...
        if (isset($save[1]) && is_array($save[1])) {
            foreach ($save[1] as $i =>$t) {
                $edit[$i] = $t;
            }
        }
		$this->db->where('id', (int)$id)->update('linkage_data_'.$key, $edit);
		$this->repair($key);
	...
	public function cache($siteid = SITE_ID) {
		...
		foreach ($linkage as $link) {
            ...
            if ($link['type']) {
                ...
                if ($siteid > 1) {
                    $cache =  $this->dcache->get('linkid-'.$siteid.'-'.$link['code']);
