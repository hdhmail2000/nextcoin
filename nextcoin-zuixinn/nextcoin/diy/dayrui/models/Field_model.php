...
class Field_model extends CI_Model {
    ...
    private function _sql_linkage($sql, $ismain) {
        $table = $this->db->dbprefix('linkage_data_'.$this->relatedid); // 表名称
        $this->db->query(str_replace('{tablename}', $table, $sql));
    }
    private function _field_linkage($tableinfo, $name) {
        $_field	= $this->db->where('fieldname', $name)->where('relatedname', 'member')->count_all_results('field');
        $_system = $tableinfo[$this->db->dbprefix('linkage_data_'.$this->relatedid)]['field'];
        return $_field ? 1 : (isset($_system[$name]) ? 1 : 0);
    }
    ...
    private function _sql_tag($sql, $ismain) {
        $table = $this->db->dbprefix($this->relatedid.'_tag'); // 表名称
        $this->db->query(str_replace('{tablename}', $table, $sql));
    }
    private function _field_tag($tableinfo, $name) {
        $_field	= $this->db->where('fieldname', $name)->where('relatedname', 'member')->count_all_results('field');
        $_system = $tableinfo[$this->db->dbprefix($this->relatedid.'_tag')]['field'];
        return $_field ? 1 : (isset($_system[$name]) ? 1 : 0);
    }
}