...
class Cron_model extends CI_Model {
    ...
    public function execute($data) {
        ...
        switch ($data['type']) {
            ...
            case 7: // 远程附件同步
                $result = $this->_execute_sync_file($data['value']);
                break;
    ...
    public function _execute_sinashare($data) {
        ...
        $call = $auth->upload(dr_strcut(str_replace('#', '、', $data['title']), 250).' '.$data['url'], $data['thumb']);
        ...
        if (@file_put_contents($path.$filename.'.'.$fileext, $file)) {
            ...
            if (is_array($result)) {
                ...
                if ($data['field']['relatedname'] == 'module') {
                    ...
                    if (strpos($table, '_draft') || strpos($table, '_verify')) {
                        ...
                        if (isset($content[$field]) && $content[$field]) {
                            $content[$field] = str_replace($data['url'].'?s=poscms', $url, $content[$field]);
                ...
                if (isset($tableinfo[$table]['field'][$field])) {
                    $db->query('UPDATE `'.$table.'` SET `'.$field.'` = replace (`'.$field.'`, "'.$data['url'].'?s=poscms", "'.$url.'")');
                } else {
                    @unlink($info['full_path']);
    ...
    public function _execute_sync_file($data) {
