...
class Weixin_model extends CI_Model {
    ...
    public function add_user($data) {
        ...
        if ($user) {
            $this->db->where('id', $user['id'])->update($this->prefix.'_user', array(
                ...
                'subscribe_time' => $data['subscribe_time'] ? $data['subscribe_time'] : SYS_TIME,
    ...
    private function _init_table($siteid) {
        // 微信表
        ...
        if (!$this->db->field_exists('content', $this->db->dbprefix($siteid.'_weixin_user')))
        {
            $this->db->query('ALTER TABLE `'.$this->db->dbprefix($siteid.'_weixin_user').'` ADD `content` TEXT NULL;');
            $this->db->query('ALTER TABLE `'.$this->db->dbprefix($siteid.'_weixin_user').'` ADD `subscribe` int(1)  NULL;');
        }
    }
