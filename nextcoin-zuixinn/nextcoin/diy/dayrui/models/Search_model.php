...
class Search_model extends CI_Model {
            ...
            if ($cat_field) {
                foreach ($cat_field as $name => $field) {
                    if (isset($get[$name]) && $get[$name]) {
                        $more = TRUE;
                        $where[] = $this->_where($table_more, $name, $get[$name], $field);
                    }
                ...
                foreach ($get as $i => $v) {
                    if (strpos($i, 'mall_') === 0) {
                        ...
                        if (isset($property[$n]) && $property[$n] && strlen($v)) {
                            $pwhere[] = '`id` IN (select `cid` from '.$this->db->dbprefix(SITE_ID.'_'.$this->dir.'_property_search').' where pid='.intval($property[$n]['id']).' and value = "'.dr_safe_replace($v).'")';
                        }
                    }
                }
            }
            // 组合查询
            $pwhere && $where.= ' AND '.implode(' and ', $pwhere);
    ...
    private function _where($table, $name, $value, $field) {
        ...
        if (strpos($value, '%') === 0
            ...
            return '`'.$table.'`.`'.$name.'` LIKE "%'.$this->db->escape_str(str_replace('\\', '\\\\\\', json_encode($value))).'%"';
