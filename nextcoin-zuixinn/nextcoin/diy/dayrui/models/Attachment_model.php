...
class Attachment_model extends CI_Model {
    ...
    public function upload($uid, $info, $id = 0) {
        ...
        // 生成缩略图
        $thumb = array();
        if ($attachinfo['width'] && function_exists('imagecreatefromstring')) {
        ...
        if (isset($remote_cfg[$this->siteid]['ext'][$_ext])
            ...
            list($remote, $file, $attachment) = $this->upload2($id, $config, $info['full_path'], array(), $thumb, $info['file_size']/1024);
        }
        ...
        if ($config['type'] == 1) {
            ...
            if ($this->ftp->connect(array(
                ...
                foreach ($locals as $local2) {
                    ...
                    if  ($is_yibu) {
                        $this->do_cron($id, array('config' => $config, 'local' => $local, '_file' => $_file));
            ...
            if ($config && $config['type'] == 1) {
                ...
                foreach ($files as $t) {
                    ...
                    if (!in_array($response->status, array(200, 204))) {
                        log_message('error', '('.$t.')阿里云存储删除失败：'.$response->status);
                    }
                }
    ...
    public function do_cron($id, $param, $block = 0) {
        ...
        $this->cron_model->add(7, array(
            'id' => $id,
            'param' => $param,
        ));

        // 生成授权文件
        file_put_contents(WEBPATH.'cache/file/'.md5($id).'.auth', dr_array2string(array(
            'time' => SYS_TIME,
            'param' => $param,
        )));
