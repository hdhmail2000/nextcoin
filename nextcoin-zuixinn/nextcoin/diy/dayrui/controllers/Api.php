...
class Api extends M_Controller {
    ...
    public function qrcode() {
        ...
        ImagePng($this->get_qrcode($this->input->get('text', true), (int)$this->input->get('uid'), $this->input->get('level', true), $this->input->get('size', true)));
        exit;
    }
        ...
        switch ($param) {
            ...
            case 'login':
                ...
                if (is_array($code)) {
                    ...
                    $data = array('msg' => fc_lang('未知错误'), 'code' => 0);
                }
                break;
            ...
            case 'get_file':
                ...
                if (!$info) {
                    $data = array('msg' => fc_lang('附件不存在'), 'code' => 0, 'url' => '');
                } else {
    ...
    public function run_remote_cron() {
        ...
        $value = dr_string2array(file_get_contents($file));
        @unlink($file);
        ...
        if ($config['type'] == 1) {
            ...
            if ($response->status == 200) {
                ...
                exit('远程附件阿里云存储模式：' . $response->body);
            }
            ...
            if ($result['code'] == 0) {
                ...
                exit('远程附件腾讯云存储模式：' . $result['message']);
            }

        } else {
            log_message('error', '远程附件类别（#'.(int)$config['type'].'）未定义');
            exit('远程附件类别（#'.(int)$config['type'].'）未定义');
        }
