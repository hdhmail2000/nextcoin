...
class Member_menu extends M_Controller {
    ...
    public function __construct() {
		...
		$this->template->assign('menu', $this->get_menu_v3(array(
		    fc_lang('会员菜单') => array('admin/member_menu/index', 'table'),
		)));
    ...
    public function index() {
		...
		if ($data) {
			foreach($data as $t) {
				...
				$t['option'] = '';
				if ($this->is_auth('admin/member_menu/add') && !@in_array($t['pid'], $left)) {
					$t['option'].= '<a class="aadd" title="'.fc_lang('添加').'" href="'.dr_dialog_url(dr_url('member_menu/add', array('pid' => $t['id'])), 'add').'"> <i class="fa fa-plus"></i> '.fc_lang('添加').'</a>';
					$t['target'] = '';
				} else {
					$t['option'].= '';
					$t['target'] = '<img src="'.THEME_PATH.'admin/images/'.$t['target'].'.gif" />';
				}
                if ($this->is_auth('admin/member_menu/edit')) {
                    $t['hidden'] = '<a href="javascript:;" onClick="return dr_dialog_set(\''.($t['hidden'] ? fc_lang('<font color=blue><b>你确定要启用它？启用后将正常使用</b></font>') : fc_lang('<font color=red><b>你确定要禁用它？禁用后将无法使用</b></font>')).'\',\''.dr_url('member_menu/hidden',array('id'=>$t['id'])).'\');"><img src="'.THEME_PATH.'admin/images/'.($t['hidden'] ? 0 : 1).'.gif"></a>';
                } else {
                    $t['hidden'] = '<img src="'.THEME_PATH.'admin/images/'.($t['hidden'] ? 0 : 1).'.gif">';
                }
				if ($this->is_auth('admin/member_menu/edit')) {
					$t['option'].= '<a class="aedit" title="'.fc_lang('修改').'" href="'.dr_dialog_url(dr_url('member_menu/edit', array('id' => $t['id'])), 'edit').'"> <i class="fa fa-edit"></i> '.fc_lang('修改').'</a>';
                    $t['name'] = '<a title="'.fc_lang('修改').'" href="'.dr_dialog_url(dr_url('member_menu/edit', array('id' => $t['id'])), 'edit').'">'.$t['name'].'</a>&nbsp;&nbsp;';
                }
				$this->is_auth('admin/member_menu/del') && $t['option'].= '<a class="adel" title="'.fc_lang('删除').'" href="javascript:;" onClick="return dr_dialog_del(\''.fc_lang('您确定要这样操作吗？').'\',\''.dr_url('member_menu/del',array('id' => $t['id'])).'\');"> <i class="fa fa-trash"></i> '.fc_lang('删除').'</a>';
				$tree[$t['id']] = $t;
			}
		}
		...
		$this->template->display('member_menu_index.html');
    }
    ...
    public function add() {
		...
		$this->template->display('member_menu_add.html');
    }
    ...
    public function init() {
		...
		$this->admin_msg(fc_lang('操作成功，正在刷新...'), dr_url('member_menu/index'), 1);
	}
