...
class Html extends D_Module {
    ...
    public function tohtml() {
        ...
        $dir = $this->input->get('dir');
        $is_root = $this->input->get('is_root');
        if ($dir) {
            ...
            if ($is_root) {
                $size = file_put_contents(WEBPATH.'index.html', $html);
                if (!$size) {
                    $this->admin_msg('生成失败，检查根目录是否可写');
                }
                if (!is_file(WEBPATH.'mobile/index.php')) {
                    $this->admin_msg('生成失败，未找到mobile目录');
                }
                // 移动端
                $html = dr_catcher_data(SITE_MURL.'index.php?cache=123');
                $size = file_put_contents(WEBPATH.'mobile/index.html', $html);
                if (!$size) {
                    $this->admin_msg('生成失败，检查移动端mobile目录是否可写');
                }
            }
