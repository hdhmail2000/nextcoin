...
class Navigator extends M_Controller {
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (isset($data['error'])) {
				...
				$this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('navigator/index', array('type' => $this->type)), 1);
			}
		} else {
            $error = '';
            $ntype = $ppid =  0;
    ...
    public function target() {
		if ($this->is_auth('admin/navigator/edit')) {
            ...
            $this->system_log('修改自定义链接【'.$this->menu[$this->type].'#'.$id.'】'); // 记录日志
			$this->cache(1);
			$this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('navigator/index', array('type' => $data['type'])), 1);
		} else {
