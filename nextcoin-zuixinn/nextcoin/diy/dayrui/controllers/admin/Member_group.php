...
class Member_group extends M_Controller {
    ...
    public function __construct() {
		...
		$this->template->assign('menu', $this->get_menu_v3(array(
			fc_lang('会员组模型') => array('admin/member_group/index', 'users'),
			fc_lang('添加') => array('admin/member_group/add', 'plus')
		)));
    ...
    public function index() {
		...
		$this->template->display('member_group_index.html');
    }
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (!$data['name']) {
				...
				$this->admin_msg(fc_lang('操作成功'), dr_url('member_group/index'), 1);
			}
		}
		...
		$this->template->display('member_group_add.html');
    }
    ...
    public function edit() {
		...
		$this->template->display($id ? 'member_group_add.html' : 'member_group_guest.html');
    }
    ...
    public function option() {
        ...
        $this->admin_msg(fc_lang('操作成功，正在刷新...'), dr_url('member_group/index'), 1);
    }
}