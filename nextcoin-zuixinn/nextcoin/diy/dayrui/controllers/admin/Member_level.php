...
class Member_level extends M_Controller {
    ...
    public function __construct() {
		...
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				...
				fc_lang('等级管理') => array('admin/member_level/index/gid/'.$this->groupid, 'signal'),
				fc_lang('添加') => array('admin/member_level/add/gid/'.$this->groupid, 'plus'),
			)),
    ...
    public function index() {
		...
		$this->template->display('member_level_index.html');
    }
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (!$data['name']) {
				...
				$this->admin_msg(fc_lang('操作成功'), dr_url('member_level/index', array('gid' => $this->groupid)), 1);
			}
		}
		$this->template->assign(array(
			'page' => $page,
		...
		$this->template->display('member_level_add.html');
    }
