...
class linkage extends M_Controller {
    ...
    public function edits() {
		...
		if (IS_POST) {
			...
			$result	= $this->linkage_model->edits($key, $id, $edit, $save);
            if (is_array($result)) {
                exit($this->admin_msg($result['error'], $result['name']));
            } else {
                $this->system_log('修改联动菜单【#'.$key.'】数据值【#'.$id.'】'); // 记录日志
                exit($this->admin_msg( fc_lang('操作成功，更新缓存生效'), dr_url('linkage/data', array('key'=>$key)), 1));
            }
		}

		$this->template->assign(array(
			'data' => $data,
			'select' => $this->select_linkage($this->linkage_model->get_list_data($link), $data['pid'], 'name=\'edit[pid]\'', fc_lang('顶级菜单')),
            'menu' => $this->get_menu_v3(array(
                fc_lang('返回') => array('admin/linkage/index', 'reply'),
                fc_lang('数据管理') => array('admin/linkage/data/key/'.$key, 'windows'),
                fc_lang('修改') => array('admin/linkage/edits/key/'.$key.'/id/'.$id, 'edit'),
            )),
	...
	public function hidden() {
		...
		$this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('linkage/data', array('key'=>$key)), 1);
	}
