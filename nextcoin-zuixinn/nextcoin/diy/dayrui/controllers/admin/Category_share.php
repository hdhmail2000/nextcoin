...
class Category_share extends M_Controller {
    ...
    protected function _get_tree($data) {
        ...
        foreach($data as $t) {
            ...
            if ($this->is_auth('admin/category_share/add') && $t['tid'] != 2) {
                $t['option'].= '<label><a class="aadd onloading" href='.dr_url('category_share/add', array('id' => $t['id'])).'> <i class="fa fa-plus"></i> '.fc_lang('子类').'</a></label>';
            }
            ...
            if ($t['tid']==1 && (!$t['child'] || $t['pcatpost']) && $this->is_auth($t['mid'].'/admin/home/add')) {
                $t['option'].= '<label><a class="aadd onloading" href='.dr_url($t['mid'].'/home/add', array('catid' => $t['id'])).'> <i class="fa fa-pencil"></i> '.fc_lang('发布').'</a></label>';
            }
            if ($t['tid']==1 && (!$t['child'] || $t['pcatpost'])) {
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if ($this->input->post('_all') == 1) {
                ...
                $this->module_model->cache();
                $this->admin_msg(fc_lang('批量添加栏目%s个，更新缓存生效', $number), dr_url('category_share/index'), 1);
            } else {
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            $this->admin_msg(fc_lang('操作成功，更新缓存生效'), $_SERVER['HTTP_REFERER'] ? $_SERVER['HTTP_REFERER'] : dr_url('category_share/edit', array('id' => $id)), 1, 1);
        }
