...
class Module extends M_Controller {
    ...
    public function config() {
		...
		if (IS_POST) {
            ...
            $this->admin_msg(fc_lang('操作成功，更新缓存生效'), $all ? dr_url('module/index') : dr_url('module/config', array('id' => $id)), 1);
		} else {
    ...
    public function copy() {
		if ($this->is_auth('admin/module/config')) {
			...
			if (IS_POST) {
                ...
                } elseif (in_array($dir, array('mall', 'order'))) {
                    exit(dr_json(0, fc_lang('此模块禁止复制'))); // 模块禁止复制
                }
				$data = $this->input->post('data');
				...
				$this->_copy_file(TPLPATH.'pc/'.SITE_TEMPLATE.'/home/'.$dir, TPLPATH.'pc/'.SITE_TEMPLATE.'/home/'.$data['dirname']);
				$this->_copy_file(TPLPATH.'mobile'.SITE_TEMPLATE.'/home/'.$dir, TPLPATH.'mobile/'.SITE_TEMPLATE.'/home/'.$data['dirname']);
				if ($data['name']) {
    ...
    public function install_all() {
        ...
        if (isset($cfg['space']) && $cfg['space']) {
            ...
            $this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('space/setting/space'), 1);
            exit;
        }
    ...
    public function install2() {
        ...
        $this->clear_cache('module');
        $this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('module/install3', array('id'=>$id, 'sid'=>$sid)), 1);
    }
    ...
    public function uninstall2() {
        ...
        $this->clear_cache('module');
		$this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('module/install', array('id'=>$id)), 1);
    }
    ...
    public function clear() {
        ...
        $this->clear_cache('module');
		$this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('module/install', array('id'=>(int)$this->input->get('id'))), 1);
    }
