...
class Weixin extends M_Controller {
    ...
    public function index() {
        ...
        if (!$this->db->query("SHOW TABLES LIKE '".$this->db->dbprefix.SITE_ID."_weixin'")->row_array()) {
            $this->admin_msg(fc_lang('首次安装后需要更新缓存生效'), dr_url('home/cache'), 2);
        }
        ...
        if (IS_POST) {
            ...
            $this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url($this->router->class.'/'.$this->router->method, array('page' => (int)$this->input->post('page'))), 1);
        }
	...
	public function ts() {
        ...
        foreach ( $data as $vo ) {
            ...
            $art ['url'] = dr_url_prefix($vo ['url'], $mid, SITE_ID);
            // 获取封面图片URL
