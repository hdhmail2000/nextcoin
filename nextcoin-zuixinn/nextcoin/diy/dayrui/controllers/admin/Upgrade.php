...
class Upgrade extends M_Controller {
    ...
    public function info() {
        ...
        $this->template->assign(array(
            'menu' => $this->get_menu_v3(array(
                fc_lang('程序信息') => array('admin/upgrade/info', 'cog'),
            )),
            'sqdomain' => dr_cms_domain_name($this->site_info[1]['SITE_URL']),
        ));
        $this->template->display('upgrande_info.html');
    }
    ...
    public function vlist() {
        ...
        $url = 'http://www.phpkaiyuancms.com/index.php?c=fc&m=newlist&my='.(DR_VERSION_ID).'&domain='.dr_cms_domain_name($this->site_info[1]['SITE_URL']);
        $data = dr_catcher_data($url);

        if (!$data) {
            exit('<p style="color:red;"> <a href="http://www.phpkaiyuancms.com/index.php?s=help&c=doc&m=bdb" target="_blank">单击前往下载补丁包！</a> </p>');
        }
    ...
    public function update() {
        ...
        define('UPVID', $vid);
        $data = dr_catcher_data(urldecode('http://www.phpkaiyuancms.com/index.php?c=fc&m=down&id='.$vid.'&sn='.$sid));
