    ...
    public function index() {
		...
		if (IS_POST) {
			...
			if (empty($post['regfield'])) {
                ...
                $this->admin_msg('Ucenter和UCSSO不能同时开启', dr_url('member_setting/index', array('page'=> $page)), 0, 9);
            }
...
{value}';
		...
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				fc_lang('功能配置') => array('admin/member_setting/index', 'cog')
			)),
			'data' => $data,
			'page' => $page,
			'result' => $result,
		...
		$this->template->display('member_setting_index.html');
    }
	...
	public function permission() {
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				fc_lang('会员权限') => array('admin/member_setting/permission', 'users'),
			))
		));
		$this->template->display('member_setting_permission.html');
	}
    ...
    public function pay() {
		...
		$this->template->assign(array(
            'pay' => $pay,
			'menu' => $this->get_menu_v3(array(
				fc_lang('网银配置') => array('admin/member_setting/pay', 'rmb'),
			)),
			'setting' => $this->member_model->pay(),
		));
		$this->template->display('member_setting_pay.html');
    }
    ...
    public function rule() {
		...
		$this->template->display('member_setting_rule.html');
    }
	...
	public function oauth() {
		...
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				'OAuth' => array('admin/member_setting/oauth', 'weibo'),
			)),
			'data' => $config,
			'oauth' => $oauth
		));
		$this->template->display('member_setting_oauth.html');
	}
    ...
    public function space() {
		...
		if (IS_POST) {
			...
			if ($post['open'] != $data['open']) {
                ...
                $this->admin_msg(fc_lang('操作成功，正在刷新...'), dr_url('member_setting/space', array('page' => $page)), 1);
            }
		}
		...
		$this->template->assign(array(
			...
			'data' => $data,
			'menu' => $this->get_menu_v3(array(
				fc_lang('空间设置') => array('admin/member_setting/space', 'cog')
			)),
			'theme' => dr_get_theme(),
		...
		$this->template->display('member_setting_space.html');
    }
