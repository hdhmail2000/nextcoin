...
class Home extends M_Controller {
                    ...
                    if (is_array($m['link'])) {
                        foreach ($m['link'] as $n) {
                            if ($n['uri'] == 'admin/odb/index'
                                && !is_file(FCPATH.'dayrui/controllers/admin/Odb.php')) {
                                continue;
                            }
                            $n['tid'] = $t['id'];
                            if (!$n['uri'] && $n['url']) {
    ...
    public function main() {
        ...
        if ($module) {
            ...
            $total['member'] = array(
                ...
                'content_verify' => $this->_set_k_url($mymenu, 'admin/member/index', 'admin/member/index/groupid/1'),
                'recycle' => 'javascript:;',
    ...
    public function cache() {
        ...
        $this->template->assign(array(
            'aurl' => $aurl,
            'list' => array(
                array('修改后台配置值、栏目、自定义内容等配置时，需要更新才会生效', 'config', 0),
                array('后台发布文章或修改文章后，前台进行实时显示时更新', 'data', 0),
                array('重建数据表字段结构，通常在自定义字段之后更新', 'table', 0),
                array('更新应用插件配置缓存', 'app', 0),
                array('更新模块搜索缓存，搜索不准时更新实时数据', 'search', 1),
                array('重新统计栏目数据量', 'category', 1),
                array('变更联动菜单数据后，更新联动菜单缓存数据', 'linkage', 1),
            )
        ));
