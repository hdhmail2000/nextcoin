...
class Odb extends M_Controller {
    ...
    public function __construct() {
        ...
        $menu = array(
            fc_lang('缓存优化') => array('admin/odb/index', 'refresh'),
            fc_lang('查询优化') => array('admin/odb/sql', 'database'),
        );
    ...
    public function index() {
        ...
        if (IS_POST) {
            $system = require WEBPATH.'config/system.php'; // 加载网站系统配置文件
            $post = $this->input->post('sys', true);

            $this->load->library('dconfig');
            $system['SYS_CACHE_TYPE'] = (int)$post['SYS_CACHE_TYPE'];
            $system['SYS_AUTO_CACHE'] =  $post['SYS_AUTO_CACHE'];

            $this->dconfig->file(WEBPATH.'config/system.php')->note('系统配置文件')->space(32)->to_require_one($this->system_model->config, $system);

            function_exists('opcache_reset') && opcache_reset();
            $this->admin_msg(fc_lang('操作成功'), dr_url('odb/index'), 1);
        }

        $this->template->assign(array(
            'type' => $type,
            'is_opcache' => function_exists('opcache_reset'),
            'is_memcached' => dr_is_memcache(),
            'is_redis' => dr_is_redis(),
            'is_wincache' => dr_is_wincache(),
            'is_apc' => dr_is_apc(),
        ));
    ...
    public function sql() {
        ...
        if (IS_POST) {
            ...
            if (!$jiexi) {
                $this->admin_msg(fc_lang('无法识别的SQL语句'));
            }
            $explain[] = array(
                'name' => '查询方式',
