...
class Field extends M_Controller {
    ...
    public function __construct() {
        ...
        if (strpos($this->relatedname, 'form') === 0) {
            ...
            $ismain = 1;
            $this->name = '会员空间字段';
            $this->backuri = 'admin/space/index'; // 返回uri地址
            $this->cacheuri = 'member'; // 缓存文件标示名称
            $this->func = 'space';
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (!$field) {
                ...
                $this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('field/index', array('rname' => $this->relatedname, 'rid' => $this->relatedid)), 1);
    ...
    public function option() {
        ...
        $this->clear_cache($this->cacheuri);
        $this->admin_msg(fc_lang('操作成功，更新缓存生效'), dr_url('admin/field/index', array('rname'=>$this->relatedname, 'rid'=>$this->relatedid)), 1);
    }
