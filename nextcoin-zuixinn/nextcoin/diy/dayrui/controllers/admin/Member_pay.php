...
class Member_pay extends M_Controller {
	...
	public function index() {
		...
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				fc_lang('财务流水') => array('admin/member_pay/'.$this->router->method, 'calculator'),
				fc_lang('添加') => array('admin/member_pay/add_js', 'plus')
			)),
			'pay' => $this->get_pay_api(1),
			'list' => $data,
			'param'	=> $param,
			'pages'	=> $this->get_pagination(dr_url('member_pay/'.$this->router->method, $param), $param['total']),
		));
		$this->template->display('member_pay_index.html');
	}
	...
	public function add() {
		...
		$this->template->display('member_pay_add.html');
	}
}