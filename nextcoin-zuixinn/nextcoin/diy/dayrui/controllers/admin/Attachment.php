...
class Attachment extends M_Controller {
	...
	public function cache() {
        ...
        if (!$page) {
            if (!SYS_AUTO_CACHE) {
                $this->admin_msg(fc_lang('系统未开启缓存'));
            } elseif (!SYS_CACHE_ATTACH) {
                $this->admin_msg(fc_lang('未设置附件缓存时间'));
            }
            $query = $this->db;
            // 统计数量
            $total = $query->count_all_results('attachment');
            if ($total) {
                $this->admin_msg(fc_lang('可生成附件%s个，正在准备执行...', $total), dr_url('attachment/cache', array('total' => $total, 'page'=>1)), 2);
            } else {
                $this->admin_msg(fc_lang('没有找到可生成的附件'));
            }
        }
        ...
        if ($page > $tpage) {
            // 更新完
            $this->admin_msg(fc_lang('已更新%s个附件', $total), NULL, 1);
        }
        $data = $this->db->limit($psize, $psize * ($page - 1))->order_by('id DESC')->get($table)->result_array();
        foreach ($data as $t) {
            get_attachment($t['id']);
        }
        $this->admin_msg(fc_lang('正在执行中(%s) ... ', "$tpage/$page"), dr_url('attachment/cache', array('total' => $total, 'page' => $page + 1)), 2, 0);
