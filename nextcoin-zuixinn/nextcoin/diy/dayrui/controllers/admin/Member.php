...
class Member extends M_Controller {
    ...
    public function index() {
		...
		if (IS_POST && $this->input->post('action')) {
			...
			if ($this->input->post('action') == 'del') {
                // 删除
                !$this->is_auth('admin/member/del') && exit(dr_json(0, fc_lang('您无权限操作')));
                foreach ($ids as $i) {
                    // 角色权限验证
                ...
                // 修改会员组
                !$this->is_auth('admin/member/edit') && exit(dr_json(0, fc_lang('您无权限操作')));
                ...
                foreach ($ids as $uid) {
                    ...
                    // 表示审核会员
                    $this->member_model->update_admin_notice('admin/member/index/field/uid/keyword/'.$uid, 3);
                }
                $this->system_log('修改会员【#'.@implode(',', $ids).'】的会员组'); // 记录日志
				exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
			}  else if($this->input->post('action') == 'user2yes'){
				foreach($ids as $ke=>$va){
					$member_info = dr_member_info($va);
					$this->db->query('update f_user set trade_status=1 where floginname='.$member_info['username']);
				}
				exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
			} else if($this->input->post('action') == 'user2no'){
				foreach($ids as $ke=>$va){
					$member_info = dr_member_info($va);
					$this->db->query('update f_user set trade_status=0 where floginname='.$member_info['username']);
				}
				exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
			} else if($this->input->post('action') == 'userAllno'){
				$this->db->query('update f_user set trade_status=0');
				exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
			} else if($this->input->post('action') == 'userAllyes'){
				$this->db->query('update f_user set trade_status=1');
				exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
			}
		}
		...
		foreach($data as $k=>$v){
			$java_member = $this->db->query('select * from f_user where `floginname`="'.$v['username'].'"')->row_array();
			$data[$k]['trade_status'] = $java_member['trade_status'];
		}
		...
		if (IS_POST) {
			...
			if (!$edit['groupid']) {
                ...
                if ($data['groupid'] != $edit['groupid']) {
                    // 表示审核会员
                    $data['groupid'] == 1 && $this->member_model->update_admin_notice('admin/member/index/field/uid/keyword/'.$uid, 3);
    ...
    public function _experience() {
        ...
        $this->template->assign(array(
            'menu' => $this->get_menu_v3(array(
                fc_lang('返回') =>  array($this->_get_back_url('member/index'), 'reply'),
                SITE_EXPERIENCE =>  array('admin/member/experience/uid/'.$uid, 'star'),
                fc_lang('添加') =>  array('admin/member/add_experience/uid/'.$uid.'_js', 'plus')
            )),
    ...
    public function experience() {
        ...
        $this->template->assign(array(
            ...
            'param'	=> $_param,
            'pages'	=> $this->get_pagination(dr_url('member/experience', $param), $param['total'])
        ));
        $this->template->display('member_score_index.html');
    }
    ...
    public function add_experience() {
        ...
        $this->template->display('member_score_add.html');
    }
    ...
    public function _score() {
        ...
        $this->template->assign(array(
            'menu' => $this->get_menu_v3(array(
                ...
                SITE_SCORE => array('admin/member/score/uid/'.$uid, 'star'),
                fc_lang('添加') => array('admin/member/add_score/uid/'.$uid.'_js', 'plus')
            )),
            'userinfo' => $this->userinfo
        ));
        $this->load->model('score_model');
