...
class Api extends M_Controller {
    ...
    public function upload() {
        ...
        $this->load->model('attachment_model');
        $my = $this->attachment_model->get_my_attach($uid, $ext);
    ...
    public function new_ajax_upload() {
        ...
        if ($this->upload->do_upload('file')) {
            ...
            list($id, $file, $_ext) = $result;
            $icon = is_file(WEBPATH.'statics/admin/images/ext/'.$_ext.'.gif') ? THEME_PATH.'admin/images/ext/'.$_ext.'.gif' : THEME_PATH.'admin/images/ext/blank.gif';
            echo json_encode(array('code'=>1, 'icon'=>$icon, 'id'=>$id, 'size'=>dr_format_file_size($info['file_size'] * 1024), 'name'=>str_ireplace(array('|', '.'.$_ext), '', $info['client_name'])));exit;
        } else {
        ...
        if ($data) {
            foreach ($data as $t) {
                if (is_dir($path.'/'.$t)) {
                    ...
                    $ext = trim(strrchr($t, '.'), '.');
                    if (in_array($ext, array('php', 'exe', 'html', 'jsp', 'asp'))) {
                        continue;
                    }
                    if (in_array('*', $exts) || in_array($ext, $exts)) {
                        $list[] = array(
                            'type' => 'file',
                            'name' => $t,
    ...
    public function checktitle() {
        ...
        $row = $this->db->where('id<>', $id)->where('title', $title)->get(SITE_ID.'_'.$module)->row_array();
        $row ? exit(fc_lang('<a href="'.$row['url'].'" style="color:red" target="_blank">'.fc_lang('已经存在').'</a>')) : exit('');
    }
    ...
    public function webchat() {
        ...
        !dr_is_app('pms') && exit($callback . '(' . json_encode(array('html' => "未安装站内信插件")) . ')');;
