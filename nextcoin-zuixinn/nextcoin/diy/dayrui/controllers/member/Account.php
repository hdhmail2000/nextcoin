...
class Account extends M_Controller {
    ...
    public function index() {
        ...
        if ($MEMBER['setting']['ismobile']) {
            if ($this->member['ismobile'] && $this->member['phone']) {
                ...
                $field['phone']['setting']['validate']['append'] =  '<label class="label label-success" style="margin-left:10px">'.fc_lang('已认证').'</label>';
            } else {
                $field['check'] = array(
                    'name' => fc_lang('短信验证码'),
                    'ismain' => 0,
                ...
                $field['phone']['setting']['validate']['append'] = '<label style="padding-left:10px"><a class="btn btn-xs blue" onclick="dr_send_sms()"> <i class="fa fa-send"></i> '.fc_lang('短信验证码').'</a></label>';
            }
        }
        ...
        if (IS_POST) {
            ...
            if (!isset($data['error']) && $this->member['groupid'] == 2) {
                ...
                if ($id == -1) {
                    ...
                    $data = array('error' => 'phone', 'msg' => fc_lang('手机号码必须是11位的整数'));
                } elseif ($id == -11) {
                ...
                if (isset($data['error'])) {
                    ...
                    $this->member_msg(fc_lang('完善资料成功'), dr_member_url('account/index'), 1);
                }
            } else {
                ...
                if ($MEMBER['setting']['ismobile'] && $v) {
                    ...
                    if (!$cache) {
                        $data = array('error' => 'check', 'msg' => fc_lang('短信验证码过期'));
                    } elseif ($cache != $data[0]['phone']) {
                        $data = array('error' => 'check', 'msg' => fc_lang('手机号码和验证码不匹配'));
                    } elseif ($this->db->where('uid<>', $this->uid)->where('phone', $data[0]['phone'])->count_all_results('member')) {
                        $data = array('error' => 'phone', 'msg' => fc_lang('该手机号码已经注册'));
    ...
    public function phone() {
        ...
        if (IS_POST) {
            ...
            // 快捷登录组完善资料
                ...
                if (isset($data['error'])) {
                    ...
                    $this->member_msg(fc_lang('操作成功，正在刷新...'), dr_member_url('account/phone'), 1);
                }
        } else {
            $data = array();
        }
        ...
        $this->template->display('account_phone.html');
    }
    ...
    public function sendsms() {
        ...
        if ($result['status']) {
            ...
            $this->session->set_userdata('send_msg_phone', $mobile);
    ...
    public function avatar() {
        ...
        if (defined('UC_KEY') && $data = uc_get_user($this->member['username']) ) {
    ...
    public function upload() {
        ...
        if (defined('UCSSO_API')) {
            ...
            $this->_avatar_return(fc_lang('通信失败：%s', $rt['msg']), $swf);
        }
    ...
    public function upgrade() {
        ...
        if ($id) {
            ...
            if ($group['unit'] == 1) {
                ...
                $this->member_model->update_score(1, $this->uid, -$value, '', '会员组升级消费：'.$group['name']);
            } else {
                // 人民币扣减
                ...
                $this->pay_model->add($this->uid, -$group['price'], '会员组升级消费：'.$group['name']);
            }
