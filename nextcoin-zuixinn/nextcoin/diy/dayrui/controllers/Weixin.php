...
class Weixin extends M_Controller {
	...
	public function sync() {
        ...
        $arr = parse_url($url);
        if ($arr['host'] && isset($domain[$arr['host']])) {
            if ($this->uid) {
                // 定向URL
		...
		if (!$user['uid']) {
			...
			if (!$oauth) {
				if ($this->wx['config']['user_type']) {
                    ...
                    $this->db->where('id', $user['id'])->update(SITE_ID.'_weixin_user', array(
                        'uid' => $uid,
                    ));
                    $user['uid'] = $uid;
	...
	private function _reply() {
		...
		if ($this->data ['MsgType'] == 'event' || $this->data ['MsgType'] == 'location') {
			...
			// 普通消息来搜索关键字库
			$msg = dr_safe_replace($this->data['Content']);
			if ($msg) {
				// 数据保存到消息管理中
				...
				// 按模块搜索
                $this->wx['reply']['module'] = array('news');
                if ($this->wx['reply']['module']) {

                    foreach ($this->wx['reply']['module'] as $dirname) {
                        foreach ($this->site as $siteid => $xs) {
                            $cache = $this->dcache->get('module-'.$siteid.'-'.$dirname);
                            if (!$cache) {
                                continue;
                            }
                            $table = $this->db->dbprefix($siteid . '_' . $dirname);
                            if (!$this->db->query("SHOW TABLES LIKE '".$table."'")->row_array()) {
                                continue;
                            }
                            // 查询关键词
                            $data = $this->db->where('`title` LIKE "%'.$msg.'%"')->order_by('id desc')->limit(8)->get($table)->result_array();
