...
function dr_member_rule($groups, $data) {
    ...
    if (IS_ADMIN) {
        ...
        $html.= '	<div class="col-md-9">';
        $html.= '	<div class="checkbox-list">';
        $html.= '		<label class="checkbox-inline"><input aid="dr_show" class="dr_show dr_select_all toggle md-check" type="checkbox" value="1"  /> '.fc_lang('全选拒绝').'</label>';
        $html.= '		<label class="checkbox-inline"><input aid="dr_add"  class="dr_add dr_select_all toggle md-check" type="checkbox" value="1" /> '.fc_lang('全选').'</label>';
        $html.= '		<label class="checkbox-inline"><input aid="dr_edit" class="dr_edit dr_select_all toggle md-check" type="checkbox" value="1" /> '.fc_lang('全选').'</label>';
        $html.= '		<label class="checkbox-inline"><input aid="dr_del" class="dr_del dr_select_all toggle md-check" type="checkbox" value="1" /> '.fc_lang('全选').'</label>';
        $html.= '	</div>';
        $html.= '	</div>';
        $html.= '</div>';
    }
	...
	foreach ($groups as $id => $group) {
		if ($id > 2) {
			...
			foreach ($group['level'] as $level) {
				...
				$html.= '	<div class="col-md-9">';
				$html.= '	<div class="checkbox-list">';
				$html.= '		<label class="checkbox-inline"><input name="rule['.$id.'][show]" class="dr_show toggle md-check" type="checkbox" value="1" '.(isset($value[$id]['show']) && $value[$id]['show'] ? 'checked' : '').' /> '.fc_lang('拒绝访问').'</label>';
				$html.= '		<label class="checkbox-inline"><input name="rule['.$id.'][add]" class="dr_add toggle md-check" type="checkbox" value="1" '.(isset($value[$id]['add']) && $value[$id]['add'] ? 'checked' : '').' /> '.fc_lang('添加').'</label>';
				$html.= '		<label class="checkbox-inline"><input name="rule['.$id.'][edit]" class="dr_edit toggle md-check" type="checkbox" value="1" '.(isset($value[$id]['edit']) && $value[$id]['edit'] ?  'checked' : '').' /> '.fc_lang('修改').'</label>';
				$html.= '		<label class="checkbox-inline"><input name="rule['.$id.'][del]" class="dr_del toggle md-check" type="checkbox" value="1" '.(isset($value[$id]['del']) && $value[$id]['del'] ? 'checked' : '').' /> '.fc_lang('删除').'</label>';
