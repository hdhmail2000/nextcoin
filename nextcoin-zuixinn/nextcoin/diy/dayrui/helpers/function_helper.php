 ...
 * 视频文件地址
 */
function dr_get_video_url($value) {
...
function dr_thumb2($img, $width = 100, $height = 100, $autocut = 1) {
   ...
   return dr_thumb($img, $width, $height);
}
...
function dr_thumb($img, $width = NULL, $height = NULL, $water = 0, $size = 0) {
    ...
    if (is_numeric($img)) { // 表示附件id
        ...
        $thumb_file = trim(SYS_THUMB_DIR, '/').'/'.md5($img).'/'.dr_safe_filename("$width-$height-$water-$size").'.jpg';
...
function dr_is_redis() {

    if (extension_loaded('redis'))
    {
        return 1;
    }
    elseif (class_exists('Redis', FALSE))
    {
        return 1;
    }
 ...
 * @return string
 */
function dr_safe_list_tag($string) {
...
// 格式化生成文件
function dr_format_html_file($file, $is_mobile = 0) {
...
function dr_post_data($url, $param, $is_file = false, $return_array = true) {
    ...
    if (class_exists('\CURLFile')) {
        foreach ($param as $key => $value) {
            if ($is_file && strpos($value, '@') === 0) {
                $file = substr($value, 1);
                if (is_file($file)) {
                    $param[$key] = new \CURLFile($file);
                }
            }
        }
    } else {
