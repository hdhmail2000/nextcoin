...
function dr_url_prefix($url, $domain = '', $siteid = SITE_ID, $is_mobile = '') {
    ...
    $is_pc = strlen($is_mobile) == 0 ? ($ci->is_mobile_html ? 0 : IS_PC) : !$is_mobile;

    // 判断是否是模块
    if ($domain && is_dir(FCPATH.'module/'.$domain.'/') && strpos($url, 'http') === false) {
        $mod = $ci->get_cache('module-'.$siteid.'-'.$domain);
        $domain = $mod && $mod['domain'] ? ($is_pc ? $mod['domain'] : $mod['mobile_domain']) : '';
    }

    // 指定域名判断
    !$domain && $domain = $siteid > 1 && $ci->site_info[$siteid]['SITE_PC'] ? ($is_pc ? $ci->site_info[$siteid]['SITE_PC'] : $ci->site_info[$siteid]['SITE_MURL']) : ($is_pc ? SITE_PC : SITE_MURL);
 ...
 * @return	string
 */
function dr_search_url($params = NULL, $name = NULL, $value = NULL, $moddir = NULL, $urlrule = NULL) {
	...
	if ($urlrule && is_dir(FCPATH.'module/'.$urlrule)) {
        // 兼容老版本，调整参数位置
        $t = $moddir;
        $moddir = $urlrule;
        $urlrule = $t;
...
function dr_space_list_url($uid, $id, $page = FALSE) {
    ...
    if ($rule) {
        $url =  dr_uri_prefix('rewrite', $space, array(), SITE_FID).str_replace(array('{uid}', '{id}', '{page}'), array($uid, $id, '[page]'), $rule);
    } else {
        $url =  dr_uri_prefix('php', $space, array(), SITE_FID).($domain ? '' : 'uid='.$uid.'&').'action=category&id='.$id.($page ? '&page=[page]' : '');
    }
    return dr_url_prefix($url, $domain);
}
...
function dr_space_show_url($uid, $mid, $id, $page = FALSE) {
    ...
    if ($rule) {
        $url =  dr_uri_prefix('rewrite', $space, array(), SITE_FID).str_replace(array('{uid}', '{id}', '{page}', '{mid}'), array($uid, $id, '[page]', $mid), $rule);
    } else {
        $url =  dr_uri_prefix('php', $space, array(), SITE_FID).($domain ? '' : 'uid='.$uid.'&').'action=show&mid='.$mid.'&id='.$id.($page ? '&page=[page]' : '');
    }
    return dr_url_prefix($url, $domain);
}
...
function dr_space_sns_url($uid, $name, $page = 0, $page2 = 0) {
    ...
    if ($name == 'show') {
        ...
        if ($rule) {
            $url =  dr_uri_prefix('rewrite', $space, array(), SITE_FID).str_replace(array('{uid}', '{id}'), array($uid, $page), $rule);
        } else {
            $url =  dr_uri_prefix('php', $space, array(), SITE_FID).($domain ? '' : 'uid='.$uid.'&').'action=sns&name='.$name.'&id='.$page;
        }
    } elseif ($name == 'topic') {
        $id = $page;
        $page = $page2;
        ...
        if ($rule) {
            ...
            $url =  dr_uri_prefix('php', $space, array(), SITE_FID).($domain ? '' : 'uid='.$uid.'&').'action=sns&name='.$name.'&id='.$id.($page ? '&page=[page]' : '');
        }
    } else {
        if ($domain) {
            // 绑定域名时的情况
        ...
        if ($rule) {
            $url =  dr_uri_prefix('rewrite', $space, array(), SITE_FID).str_replace(array('{uid}', '{name}', '{page}'), array($uid, $name, '[page]'), $rule);
        } else {
            $url =  dr_uri_prefix('php', $space, array(), SITE_FID).($domain ? '' : 'uid='.$uid.'&').'action=sns&name='.($page ? '&page=[page]' : '');
        }
    }
 ...
 * @return	array
 */
function dr_search_seo($mod, $catid, $param, $page = 1) {
    ...
    $param_value = array();
    if ($catid) {
        $param_value = explode(PHP_EOL, dr_get_cat_pname($mod, $mod['category'][$catid], PHP_EOL));
        unset($param['catid']);
    }
    if ($param) {
        if ($catid && isset($mod['category'][$catid]['field']) && $mod['category'][$catid]['field']) {
            $fields = array_merge($mod['field'], $mod['category'][$catid]['field']);
        } else {
            $fields = $mod['field'];
        }
    ...
    $seo['meta_title'] = dr_safe_replace($seo['meta_title']);
    $seo['meta_keywords'].= $mod['site'][SITE_ID]['meta_keywords'];
    $seo['meta_keywords'] = dr_safe_replace(trim($seo['meta_keywords'], ','));
...
// 模块URL
function dr_module_url($mod, $sid = SITE_ID) {


    $ci	= &get_instance();
    if (!is_array($mod)) {
        $mod = $ci->get_cache('module-'.$sid.'-'.dr_safe_filename($mod));
    }
