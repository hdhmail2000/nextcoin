...
class F_Property2 extends A_Field {
    ...
    public function option($option) {
        return '<div class="form-group">
				...
				<div class="col-md-9" style="color:blue"> <div class="form-control-static">'.fc_lang('此字段只能用于商城模块mall的模块字段').'</div></div>
			</div>
			';
    }
	...
	public function insert_last_value($cid, $value, $field) {
		...
		// 删除原数据
		$this->ci->db->where('cid', $cid)->delete($table);
		...
		if ($value) {
			...
			foreach ($value as $name => $t) {
				if (isset($property[$name]) && $property[$name] && (@strlen($t) || is_array($t))) {
					...
					if (is_array($t)) {
						foreach ($t as $v) {
							strlen($v) && $this->ci->db->insert($table, array(
								'cid' => $cid,
								'pid' => $pid,
								'value' => $v,
							));
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		function dr_load_mall_property() {
			...
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "/index.php?s=member&mod=mall&c=api&m=property",
				data: {catid:$("#dr_catid").val(), value: '.json_encode($value).', name: "'.$name.'"},
				success: function(json) {
					$("#dr_'.$name.'_result").html(json.code);
					if (json.status == 1) {
						$("#dr_row_'.$name.'").show();
					} else {
						$("#dr_row_'.$name.'").hide();
