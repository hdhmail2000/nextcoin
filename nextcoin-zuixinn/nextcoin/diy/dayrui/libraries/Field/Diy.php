...
class F_Diy extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		if ($cfg['option']['type']) {
            ...
            if ($cfg['option']['code']) {
                // 代码安全验证
                if (strpos($cfg['option']['code'], '<?') !== false
                    || strpos($cfg['option']['code'], '{php') !== false
                    || strpos($cfg['option']['code'], '<?php') !== false) {
                    $code = '<font color=red>代码模式中不能包含PHP标签，请使用文件模式</font>';
                } else if (strpos($cfg['option']['code'], 'eval') !== false
                    || strpos($cfg['option']['code'], '$_') !== false) {
                    $code = '<font color=red>代码模式中不能包含敏感变量或函数，请使用文件模式</font>';
                } else {
                    ob_start();
                    $file = $this->ci->template->code2php($cfg['option']['code']);
                    require_once $file;
                    $code = ob_get_clean();
                    if (strpos($code, '<!--POSCMS error-->')) {
