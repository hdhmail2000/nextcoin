...
class F_Video extends A_Field {
    ...
    public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        $my = '<button type="button" style="cursor:pointer;" class="btn red btn-sm" onclick="dr_add_video_'.$name.'()"> <i class="fa fa-flag"></i> '.fc_lang('提示点').'</button>';
        if ($value['file']) {
            $file = $value['file'];
            $data = dr_file_info($file);
            if ($data) {
                $my.= '
					<button type="button" style="cursor:pointer;pointer;margin-left: 6px;" class="btn green btn-sm" onclick="dr_show_file_info(\''.$data['id'].'\')"> <i class="fa fa-search"></i> '.fc_lang('预览').'</button>
        ...
        if (!$disabled) {
            ...
            $str.= '<div style="margin-left: 10px;" class="col-md-3 my_upload"><button type="button" style="cursor:pointer;"  class="btn blue btn-sm" onclick="dr_upload_file(\''.$name.'\', \''.$furl.'\')"> <i class="fa fa-folder"></i> ' . fc_lang('浏览') . '</button></div>';
