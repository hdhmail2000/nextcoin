		...
		$disabled = !IS_ADMIN && $id && $value && isset($cfg['validate']['isedit']) && $cfg['validate']['isedit'] ? 'disabled' : '';
			...
			if ($value) {
				...
				if ($data) {
					...
					<button type="button" style="cursor:pointer;margin-left: 6px;"  class="btn red btn-sm" onclick="dr_delete_file2(\''.$name.'\')"> <i class="fa fa-trash"></i> ' . fc_lang('删除') . '</button>
					';
			...
			if (!$disabled) {
				...
				<div id="drag-and-drop-zone-'.$name.'" class="col-md-3 btn btn-sm blue uploader">
					<i class="fa fa-cloud-upload"></i> '.fc_lang('上传').' <input type="file" name="file">
				</div>';

                $str.= '<div style="margin-left: 10px;margin-right:15px" class="col-md-3 my_upload"><button type="button" style="cursor:pointer;"  class="btn blue btn-sm" onclick="dr_upload_file(\''.$name.'\', \''.$furl.'\')"> <i class="fa fa-folder"></i> ' . fc_lang('浏览') . '</button></div>';
			...
			$str.= '
          	...
          	<div style="padding-top: 10px;clear: both;">格式要求：'.$cfg['option']['ext'].'，最多上传 1 个文件，单文件最大 '.$cfg['option']['size'].' MB</div>
      ...
      $("#drag-and-drop-zone-'.$name.'").dmUploader({
        ...
        onUploadSuccess: function(id, data){
		  if (data.code == 1) {
			...
			$("#dr_my_'.$name.'_list").append("<button type=\"button\" style=\"cursor:pointer;\" class=\"btn red btn-sm\" onclick=\"dr_delete_file2(\''.$name.'\')\"> <i class=\"fa fa-trash\"></i> '.fc_lang('删除').'</button> ");
		  } else {
		  	dr_tips(data.msg,5);
