...
class F_Files extends A_Field {
	...
	public function option($option) {
		...
		return '
	        ...
	        <div class="form-group">
				<label class="col-md-2 control-label">'.fc_lang('提取首图作为缩略图').'：</label>
				<div class="col-md-9">
				<div class="radio-list">
					<label class="radio-inline"><input type="radio" value="1" name="data[setting][option][autothumb]" '.($option['autothumb'] ? 'checked' : '').' > '.fc_lang('启用').'</label>
					<label class="radio-inline"><input type="radio" value="0" name="data[setting][option][autothumb]" '.(!$option['autothumb'] ? 'checked' : '').' > '.fc_lang('关闭').'</label>
				</div>
				</div>
			</div>
			<div class="form-group">
	...
	public function insert_value($field) {
		...
		// 第一张作为缩略图
		if (isset($field['setting']['option']['autothumb'])
            && $field['setting']['option']['autothumb']
            && isset($_POST['data']['thumb']) && !$_POST['data']['thumb']
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		//(IS_ADMIN && IS_PC) || (isset($cfg['option']['is_swfupload']) && $cfg['option']['is_swfupload'])
			...
			if (!$disabled) {
				$str.= '<div id="drag-and-drop-zone-'.$name.'" class="btn btn-sm blue uploader" style="cursor: pointer;"><i class="fa fa-cloud-upload"></i> '.fc_lang('上传').' <input type="file" name="file" multiple=""></div>';
                $str.= '<a style="margin-left: 10px;margin-right:15px" class="btn blue btn-sm" href="javascript:;" onClick="dr_upload_files(\'' . $name . '\',\'' . $url . '\', \'\', \'' . (int)$cfg['option']['count'] . '\')"> <i class="fa fa-folder"></i> ' . fc_lang('浏览') . '</a>&nbsp;&nbsp;';
                $str.= '<div style=";margin-top: 6px;">格式要求：'.$cfg['option']['ext'].'，最多上传  '.$cfg['option']['count'].' 个文件，单文件最大 '.$cfg['option']['size'].' MB</div>';
                $str.= '
				  ...
				  $("#drag-and-drop-zone-'.$name.'").dmUploader({
					...
					onUploadSuccess: function(id, data){
					  ...
					  if (data.code == 1) {
					  	 var name = "'.$name.'";
					 	 var c = "";
						  c += \'<input type="hidden" value="\' + data.id + \'" name="data['.$name.'][file][]" id="fileid_'.$name.'_\' + mid + \'" />\';
						  c += \'<label><input type="text" class="form-control" style="width:300px;height:30px;" value="\' + data.name + \'" name="data['.$name.'][title][]" /></label>\t\';
						  c += \'<label><a href="javascript:;" class="btn btn-sm red" onclick="dr_remove_file(\\\'\' + name + \'\\\',\\\'\' + mid + \'\\\')">\';
						  c += \'<i class="fa fa-trash"></i></a></label>\t\';
						  c += \'<label id="span_\' + name + \'_\' + mid + \'"><a href="javascript:;" class="btn btn-sm blue" onclick="dr_show_file_info(\\\'\' + data.id + \'\\\')\">\';
					...
					onUploadError: function(id, message){
					  alert("文件上传失败 #" + id + ": " + message);
					  var mid = dr_'.$name.'_ids[id];
					  $("#files_'.$name.'_"+mid).html("");
					},
					onFileTypeError: function(file){
					  alert("文件 \"" + file.name + "\" 必须要求是图片");
					  var mid = dr_'.$name.'_ids[id];
					  $("#files_'.$name.'_"+mid).html("");
					},
					onFileSizeError: function(file){
					  alert( "文件 \"" + file.name + "\" 扩展名不允许");
					  var mid = dr_'.$name.'_ids[id];
					  $("#files_'.$name.'_"+mid).html("");
					},
