...
class F_Template extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        if (APP_DIR && APP_DIR != 'member' && is_dir(FCPATH.'module/'.APP_DIR)) {
            // 模块
            $files = directory_map(TPLPATH.'pc/'.SITE_TEMPLATE.'/home/'.APP_DIR.'/', 1);
            ...
            if (defined('IS_SPACE_THEME')) {
                ...
                // 会员中心
                $files = directory_map(TPLPATH.'pc/'.SITE_TEMPLATE.'/member/', 1);
            }
        } else {
            // 网站
            $files = directory_map(TPLPATH.'pc/'.SITE_TEMPLATE.'/home/', 1);
        }
        if ($files) {
			foreach ($files as $t) {
				if ($t && strpos($t, '.html')) {
