...
class D_Admin_Home extends M_Controller {
        ...
        if (IS_POST && !$this->input->post('search')) {
            ...
            switch ($action) {
                case 'del':
                    ...
                    exit(dr_json($no ? 0 : 1, $no ? fc_lang('删除成功：%s，失败：%s', $ok, $no) : fc_lang('操作成功，正在刷新...')));
                    break;
                case 'recycle':
                    $ok = $no = 0;
                    foreach ($ids as $id) {
                        ...
                        if ($data) {
                            if (!$this->is_category_auth($data['catid'], 'del')) {
                                ...
                                $ok++;
                                $this->content_model->recycle_for_id((int)$data['id'], (int)$data['tableid']);
                            }
                        }
                    }
                    $this->system_log('回收站 站点【#'.SITE_ID.'】模块【'.APP_DIR.'】内容【#'.@implode(',', $ids).'】'); // 记录日志
                    exit(dr_json($no ? 0 : 1, $no ? fc_lang('回收成功：%s，失败：%s', $ok, $no) : fc_lang('操作成功，正在刷新...')));
                    break;
                case 'order':
    ...
    public function edit() {
        ...
        // 判断栏目权限
        !$this->is_category_auth($catid, 'edit') && $this->admin_msg(fc_lang('您无权限操作'));
    ...
    public function recycle() {
        ...
        $table = $this->content_model->prefix.'_recycle';
        ...
        if (IS_POST) {
            ...
            if ($this->input->post('action') == 'del') {
                foreach ($ids as $id) {
                    $data = $this->db->where('id', (int)$id)->get($this->content_model->prefix.'_recycle')->row_array();
                    if ($data) {
                        $c = dr_string2array($data['content']);
                        $this->content_model->delete_for_id((int)$c['id'], (int)$c['tableid']);
                    }
                }
                ...
                foreach ($ids as $id) {
                    ...
                    if ($data) {
                        $c = dr_string2array($data['content']);
                        $this->content_model->recycle_add((int)$c['id'], $c);
                    }
                }
        ...
        // 存储当前页URL
        $this->_set_back_url(APP_DIR.'/home/index', '', APP_DIR.'/home/recycle');

        $this->template->assign(array(
            'menu' => $this->get_menu_v3(array(
                fc_lang('回收站') =>  array(APP_DIR.'/admin/home/recycle', 'trash'),
                fc_lang('发布') => array(APP_DIR.'/admin/home/add', 'plus')
            )),
            'list' => $result,
            'total' => $total,
            'pages' => $this->get_pagination(dr_url(APP_DIR.'/home/recycle'), $total)
        ));
    ...
    public function recycleedit() {
        ...
        $id = (int)$this->input->get('id');
        $data = $this->content_model->get_recycle($id);
        $error = array();
        ...
        $backuri = APP_DIR.'/admin/home/recycle/';
        ...
        $this->template->assign(array(
            ...
            'menu' => $this->get_menu_v3(array(
                ...
                fc_lang('查看') => array(APP_DIR.'/admin/home/recycleedit/id/'.$data['id'], 'edit')
            )),
            'catid' => $catid,
            'error' => $error,
            'select' => $this->select_category($category, $catid, 'id=\'dr_catid\' name=\'catid\' onChange="show_category_field(this.value)"', '', 1, 1),
        ...
        if ($this->admin['adminid'] == 1) {
            ...
            if ($category) {
                ...
                foreach ($category as $c) {
                    !$c['child'] && $c['setting']['admin'][$this->admin['adminid']]['show'] == 1 && $catids[] = $c['id'];
                }
                $catids && $where.= ' and `catid` IN ('.implode(',', $catids).')';
            ...
            $_menu[fc_lang('我的审核')] = array(
                ...
                'count' => $this->db->where($where)->count_all_results($this->content_model->prefix.'_verify')
            );
