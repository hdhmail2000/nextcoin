...
class C_Model extends CI_Model {
     ...
     * @return	array	
     */
    protected function _where(&$select, $data) {
     ...
     * @return	array	
     */
    protected function _extend_where(&$select, $data, $cid) {
    ...
    public function index($data) {
        ...
        if (defined('IS_SHARE') && IS_SHARE) {
            ...
            $this->db->replace($this->prefix.'_index', array(
                'id' => $id,
                'uid' => $data[1]['uid'],
                'catid' => $data[1]['catid'],
    ...
    public function add($data, $syncatid = 0) {
        ...
        if ($data[1]['status'] >= 9) {
            ...
            $verify = array(
                ...
                'author' => $data[1]['author'] ? $data[1]['author'] : 'guest',
     ...
     * @return	array
     */
    protected function extend_index($data) {
    ...
    // 筛选出栏目表字段
    protected function replace_category_data($id, $data) {
    ...
    public function get_recycle($id) {
        ...
        $data1 = dr_string2array($row['content']);
        // 副表
        $data2 = $this->db->where('id', $id)->limit(1)->get($this->prefix.'_data_'.$data1['tableid'])->row_array();
        // 栏目附加数据
        $data3 = $this->db->where('id', $id)->limit(1)->get($this->prefix.'_category_data')->row_array();
    ...
    public function sns_share($body, $is_extend = 0) {
        ...
        $url = dr_url_prefix($data['url'], $this->mdir, SITE_ID); // 地址
        $uid = $data['uid'] ? $data['uid'] : $this->uid;
        $thumb = $data['thumb'] ? dr_thumb($data['thumb']) : '';
        $title = dr_clearhtml($data['title'].($data['description'] ? ' '.$data['description'] : ''));
    ...
    public function delete_for_id($id, $tableid) {
        ...
        // 删除表对应的附件
        if ($this->ci->get_cache('module-'.SITE_ID.'-'.$this->mdir, 'setting', 'attdel')) {
    ...
    public function recycle_for_id($id, $tableid) {
        ...
        if ($this->ci->get_cache('module-'.SITE_ID.'_'.$this->mdir, 'extend')) {
            $this->db->where('cid', $id)->update($this->prefix.'_extend_index', array(
                'status' => 10,
            ));
            $this->db->where('cid', $id)->update($this->prefix.'_extend', array(
                'status' => 10,
            ));
        }
