    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if (isset($data['error'])) {
                ...
                $data[1]['hits'] = 0;
                $data[1]['catid'] = $catid;
                $data[1]['status'] = $this->module_rule[$catid]['verify'] ? 1 : 9; // 没有设置审核流程就直接发布
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            if (isset($data['error'])) {
                ...
                if ($this->module_rule[$catid]['edit_verify']) {
                    // 修改时不审核
                    $data[1]['status'] = max((int)$_data['status'], 1);
                } else {
                    $data[1]['status'] = $this->module_rule[$catid]['verify'] ? 1 : $_data['status'];
                }
