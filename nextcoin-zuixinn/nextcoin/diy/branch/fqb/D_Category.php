...
class D_Category extends M_Controller {
    ...
    protected function _get_tree($data) {
        ...
        $category = $this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'category');
        foreach($data as $t) {
            $url = dr_url_prefix($category[$t['id']]['url'] ? $category[$t['id']]['url'] : '/index.php?s='.APP_DIR.'&c=category&id='.$t['id'], $domain);
            ...
            $this->is_auth(APP_DIR.'/admin/category/add') && $t['option'].= '<label><a class="aadd onloading" href='.dr_url(APP_DIR.'/category/add', array('id' => $t['id'])).'> <i class="fa fa-plus"></i> '.fc_lang('子类').'</a></label>';
