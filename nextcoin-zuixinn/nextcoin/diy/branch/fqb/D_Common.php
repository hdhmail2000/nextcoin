...
class D_Common extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->_init_variable();
        $this->lang->load('my');
        $this->hooks->call_hook('finecms-init'); // poscms 初始化完毕的钩子
        $this->hooks->call_hook('poscms-init'); // poscms 初始化完毕的钩子
    ...
    public function clear_cache($name) {
        ...
        if ($name == 'module') {
            $obj = null;
            if (defined('SYS_CACHE_TYPE') && SYS_CACHE_TYPE) {
                switch (SYS_CACHE_TYPE) {
                    case 1:
            ...
            if ($data && $obj) {
                foreach ($data as $site => $t) {
                    if ($t) {
                        foreach ($t as $m) {
                            $obj->delete(SYS_KEY.'module-'.$site.'-'.$m);
                        }
                    }
                }
            }
    ...
    public function get_cache_data($name) {
        ...
        if (!$name) {
            ...
            // 禁用缓存
            return NULL;
        } elseif (defined('SYS_CACHE_TYPE') && SYS_CACHE_TYPE) {
            switch (SYS_CACHE_TYPE) {
                case 1:
                    if (dr_is_memcache() && $this->cache->memcached->is_supported()) {
                        return $this->cache->memcached->get(SYS_KEY.$name);
                    }
                    break;
                case 2:
                    if (dr_is_redis() && $this->cache->redis->is_supported()) {
                        return $this->cache->redis->get(SYS_KEY.$name);
                    }
                    break;
                case 3:
                    if (dr_is_wincache() && $this->cache->wincache->is_supported()) {
                        return $this->cache->wincache->get(SYS_KEY.$name);
                    }
                    break;
                case 4:
                    if (dr_is_apc() && $this->cache->apc->is_supported()) {
                        return $this->cache->apc->get(SYS_KEY.$name);
                    }
                    break;
            }
        }
    ...
    public function set_cache_data($name, $data, $ttl = 10) {
        ...
        if (!$name || !$ttl) {
            ...
            switch (SYS_CACHE_TYPE) {
                case 1:
                    if (dr_is_memcache() && $this->cache->memcached->is_supported()) {
                        $this->cache->memcached->save(SYS_KEY.$name, $data, $ttl);
                        return $data;
                    }
                    break;
                case 2:
                    if (dr_is_redis() && $this->cache->redis->is_supported()) {
                        $this->cache->redis->save(SYS_KEY.$name, $data, $ttl);
                        return $data;
                    }
                    break;
                case 3:
                    if (dr_is_wincache() && $this->cache->wincache->is_supported()) {
                        $this->cache->wincache->save(SYS_KEY.$name, $data, $ttl);
                        return $data;
                    }
                    break;
                case 4:
                    if (dr_is_apc() && $this->cache->apc->is_supported()) {
                        $this->cache->apc->save(SYS_KEY.$name, $data, $ttl);
                        return $data;
                    }
                    break;
            }
    ...
    private function _init_variable() {
        ...
        if (is_file(WEBPATH.'config/client_domain.php')) {
            $this->client_domain = require WEBPATH.'config/client_domain.php';
        }
        foreach ($sitecfg as $file) {
            $id = (int)basename($file);
            if (is_file(WEBPATH.'config/site/'.$file) && $id > 0) {
        ...
        if (isset($domain[DOMAIN_NAME]) && isset($this->site_info[$domain[DOMAIN_NAME]])) {
            ...
            if ($orthers && DOMAIN_NAME != $this->site_info[$siteid]['SITE_DOMAIN'] && in_array(DOMAIN_NAME, $orthers)) {
                // 判断当前域名为“其他域名”
                // 301 转向开启
                if ($this->site_info[$siteid]['SITE_URL_301']) {
            ...
            } elseif (isset($this->client_domain[DOMAIN_NAME]) && $this->client_domain[DOMAIN_NAME]) {
                // 来自终端
                !defined('SITE_CLIENT_ID') && define('SITE_CLIENT_ID', $this->client_domain[DOMAIN_NAME]);
        ...
        if (IS_ADMIN && is_file(WEBPATH.'config/admin_domain.php')) {
            ...
            if (isset($admin_domain[SITE_ID]) && $admin_domain[SITE_ID]) {
                if (dr_cms_domain_name($admin_domain[SITE_ID]) != dr_cms_domain_name(DOMAIN_NAME)) {
                    // 非后台域名时,禁止访问
                    if ($config3['SYS_DEBUG']) {
                        show_error('请使用后台专用域名['.$admin_domain[SITE_ID].']登陆后台', 200, '后台域名错误');
                    } else {
                        show_error('请使用后台专用域名登陆后台', 200, '后台域名错误');
                    }
                }
        ...
        if (!$config3['SYS_UPLOAD_DIR']) {
            ...
            $config3['SYS_ATTACHMENT_URL'] = $config3['SYS_ATTACHMENT_URL'] ? $config3['SYS_ATTACHMENT_URL'] : $config3['SITE_PC'].$config3['SYS_UPLOAD_DIR'];
        } else {
        ...
        define('THEME_PATH', SITE_PC. 'statics/');
...
//        define('HOME_THEME_PATH', strpos(SITE_THEME, 'http') === 0 ? trim(SITE_THEME, '/').'/' : THEME_PATH.SITE_THEME.'/');
        define('HOME_THEME_PATH', 'https://www.nextcoin.vip/statics/default/');
        ...
        if (IS_MEMBER) {
            ...
            // 后台钩子
            $this->hooks->call_hook('poscms-admin', $uri);
    ...
    public function is_auth($uri) {
        ...
        if (in_array($route, array(
            'space/admin/content/edit',
            'space/admin/content/del',
            'space/admin/content/add',
          ))) {
            if (in_array('space/admin/content/index/mid/'.$_GET['mid'], $role)) {
                return TRUE;
            } else {
                return FALSE;
            }
    ...
    public function get_member_pagination($url, $total) {
        ...
        $config = array();
        is_file(WEBPATH.'config/pagination_member.php') ? include WEBPATH.'config/pagination_member.php' : include WEBPATH.'config/pagination.php';
    ...
    public function callback_json($data) {
        ...
        // 自定义返回名称
        $return = dr_safe_replace($this->input->get('return', true));
    ...
    protected function validate_filter($_field, $_data = array()) {
        ...
        foreach ($_field as $field) {
            ...
            if ($field['fieldtype'] == 'Ueditor' && strlen($this->post[$name]) > 1000000) {
                return array('error' => $name, 'msg' => fc_lang('%s内容太多', $field['name']));
            }
            // 编辑器默认关闭xss
                    ...
                    if (method_exists($this->dvalidate, $method)) {
                        ...
                        if ('check_member' == $method && $value == 'guest') {
                            ...
                            if (call_user_func_array(array($this->dvalidate, $method), $param)) {
                                $msg = $validate['errortips'] ? $validate['errortips'] : $field['name'].'：'.fc_lang('格式校验不正确');
                                return array('error' => $name, 'msg' => $msg);
                            }
                        }
                    } else {
...
{value}';
    ...
    protected function api_synlogin() {
        ...
        ob_start();
        $code = substr(md5(SYS_KEY.$member['password']), 5, 20);
        set_cookie('member_uid', $uid, $expire);
        set_cookie('member_cookie', $code, $expire);
        ob_get_clean();
    ...
    protected function _admin_search_field($field) {

        if ($field) {
            foreach ($field as $i => $t) {
                if (!$t['fieldtype']) {
                    continue;
                } elseif (!in_array($t['fieldtype'], array(
                    'Text',
                    'Textarea',
                    'Ueditor',
                    'Select',
    ...
    protected function _module_init() {
        ...
        // 判断是否拒绝使用
        !IS_ADMIN && isset($this->module['setting']['member'][$this->markrule]) && $this->msg(fc_lang('无权限使用此模块'));
    ...
    public function replace_lang($string) {
        ...
        if ($this->replace_lang === 1) {
            ...
            is_file(FCPATH.'dayrui/language/'.SITE_LANGUAGE.'/replace_lang.php') && require FCPATH.'dayrui/language/'.SITE_LANGUAGE.'/replace_lang.php';
    ...
    public function html_thumb($p) {
        ...
        } elseif ($info['fileext'] == 'gif') {
            // gif格式
            return dr_get_file($id);
        }

        // 缓存文件
        $thumb_file = trim(SYS_THUMB_DIR, '/').'/'.md5($id).'/'.dr_safe_filename("$width-$height-$water-$size").'.jpg';
        dr_mkdirs(WEBPATH.dirname($thumb_file));

        if ($info && in_array($info['fileext'], array('jpg', 'png', 'jpeg'))) {
            // 远程图片下载到本地缓存目录
        ...
        // 水印判断
        if (isset($info['remote']) && in_array($info['fileext'], array('jpg', 'png', 'jpeg')) && $info['remote'] && !$site['SITE_IMAGE_REMOTE']
    ...
    // 接口认证
    protected function _api_nologin_auth($auth, $t) {

        if ($t) {
            // 移动端验证
            if ($auth != md5(SYS_REFERER)) {
                // 授权认证码不正确
                $data = array('msg' => '移动端api密钥-不正确', 'code' => 0);
            ...
            if ($auth != md5(SYS_KEY)) {
                // 授权认证码不正确
                $data = array('msg' => '网站密钥-不正确', 'code' => 0);
                $this->_api_nologin_call($data);
            }
        }
    }
    ...
    // 查询接口返回
    protected function _api_nologin_call($data) {
        // 接收参数
        $format = $this->input->get('format', true);
        $function = $this->input->get('function', true);
        if ($function) {
            if (strpos($function, 'dr_') !== 0) {
        ...
        if ($format == 'php') {
            ...
            // 自定义返回名称
            echo dr_safe_replace($this->input->get('callback', TRUE)).'('.$this->callback_json($data).')';
        } else {
            // 自定义返回名称
