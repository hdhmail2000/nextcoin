...
class D_Home_Form extends M_Controller {
    ...
    public function __construct() {
        ...
        // 投稿权限验证
        $this->mform_rule = $this->form['permission'][$this->markrule];
        // 禁用权限
        $this->mform_rule['disabled'] && $this->msg(fc_lang('当前会员组无权限操作'));
    }
    ...
    protected function _get_data($id) {
        ...
        $data2 = $this->db->where('id', $id)->get($this->table.'_data_'.(int)$data['tableid'])->row_array();
        if ($data2) {
    ...
    public function Page() {
        ...
        $this->template->assign(array(
            ...
            'urlrule' => 'index.php?s='.APP_DIR.'&c=form_'.$this->form['table'].'&m=page&cid='.$data['id'].'&page=[page]',
            'meta_title' => ($page > 1 ? fc_lang('第%s页', $page).SITE_SEOJOIN : '').$this->form['name'].SITE_SEOJOIN.$data['title'].SITE_SEOJOIN.$cat['name'].SITE_SEOJOIN.MODULE_NAME.SITE_SEOJOIN.SITE_NAME
        ));
        $tpl = dr_tpl_path('form_'.$this->fid.'_page.html');
