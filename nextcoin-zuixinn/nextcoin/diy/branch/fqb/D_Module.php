...
class D_Module extends D_Common {
    ...
    protected function _show_buy() {
        ...
        if (!$data['is_buy']
            ...
            // 卖家增加
            $data['uid'] && $this->member_model->update_score(1, $this->uid, $data['uid'], '', '卖出《'.($data['name'] ? $data['name'] : $data['title']).'》');
            // 记录购买历史
            $insert = array(
                'uid' => $this->uid,
                'url' => $data['url'],
    ...
    protected function _search($call = '') {
        ...
        if ($call) {
            ...
            $urlrule = dr_search_url($get, 'page', '{page}', $this->dir );
    ...
    protected function _show_html() {
        ...
        $time_start = (int)$this->input->get('time_start');
        $time_end = (int)$this->input->get('time_end');
        $id_start = (int)$this->input->get('id_start');
        $id_end = (int)$this->input->get('id_end');
        ...
        if (IS_POST) {
            ...
            foreach ($cat as $id) {
                ...
                if ($type == 'html') {
                    $category[$id]['setting']['html'] && $catid[] = (int)$id;
                } else {
                    $catid[] = (int)$id;
                }
            }
        ...
        if (!$page) {
            $url.= '&p=1&catid='.$catid.'&id_start='.$id_start.'&id_end='.$id_end.'&time_start='.$time_start.'&time_end='.$time_end.'&type='.$type.'&value='.$value;
            $url.= '&type='.$type;
        ...
        if ($page == 1 && !$total) {
            ...
            if ($time_start || $time_end) {
                $this->db->where('`inputtime` between '.($time_start ? strtotime($time_start) : 1).' and '.($time_end ? strtotime($time_end) : SYS_TIME));
            }
            if ($id_start || $id_end) {
                $this->db->where('`id` between '.($id_start ? $id_start : 1).' and '.($id_end ? $id_end : SYS_TIME));
            }
            $value && $this->db->where('`id` IN ('.$value.')');
            $total = $this->db->count_all_results(SITE_ID.'_'.$this->dir.'_index');
            !$total && $this->admin_msg("无可用数据");
            $msg = '共 '.$total.' 条数据...';
            $url = $url.'&p=1&total='.$total.'&catid='.$catid.'&id_start='.$id_start.'&id_end='.$id_end.'&time_start='.$time_start.'&time_end='.$time_end.'&type='.$type.'&value='.$value;
        ...
        if ($type == 'html') {
            ...
            $url = $url.'&p='.$next.'&total='.$total.'&catid='.$catid.'&id_start='.$id_start.'&id_end='.$id_end.'&time_start='.$time_start.'&time_end='.$time_end.'&type='.$type.'&value='.$value;
            $this->admin_msg($msg, $url, 2, 0);
        } else {
            $pagesize = 500;// 每次生成数量
            $count = ceil($total/$pagesize); // 计算总页数
     ...
     * 创建栏目的html文件
     */
    protected function _create_category_file($catid, $dir = '', $page = 0, $is_mobile = 0) {
