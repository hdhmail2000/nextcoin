...
var util = {
	...
	showconfirm : function(value, options) {
		var defaults = {
			...
			oktxt : util.getLan("comm.tips.19"),
			notxt : util.getLan("comm.tips.20"),
			html : false,
			noshow : false,
			okbtn : function() {
	...
	recordTab : function(ele) {
		...
		if (value == 0) {
			value = 1;
			title = language["comm.tips.15"] + "&nbsp;+";
			$("#recordbody" + type).hide();
		} else {
			value = 0;
			title = language["comm.tips.16"] + "&nbsp;-";
	...
	isPassword : function(pwd) {
		...
		if (pwd == "") {
			desc = language["trade.tips.2"];
		}
		// else if (pwd.length < 6) {
		// 	desc = language["comm.tips.12"];
		// } else if (pwd.length > 16) {
		// 	desc = language["comm.tips.14"];
		// } else if (!c.test(pwd)) {
		// 	desc = language["comm.tips.13"];
		// }
		return desc;
	},
	...
	globalurl : {
		DEFAULT : {
			...
			ticker : "/index.php?s=exc&c=indexController&m=indexMarket",
			klineType : {},
			isremote : false,
		}
//		DEFAULT : {
//			market : "/index.php?s=exc&c=TradeController&m=market&symbol={0}&buysellcount={1}&successcount={2}",
//			kline : "/index.php?s=exc&c=indexController&m=kline",
//			depth : "/index.php?s=exc&c=tradeController&m=fulldepth",
//			ticker : "/index.php?s=exc&c=indexController&m=indexMarket",
//			klineType : {},
//			isremote : false,
//		}
	...
	layerConfirm : function(content, callback) {
		var options = {
			...
			maxWidth : 1000,
            title : util.getLan("comm.tips.18"),
            btn: [util.getLan("comm.tips.19"), util.getLan("comm.tips.20")]
		}
	...
	network : function(options) {
		...
		// options.url = '/ajax.php?url='+encodeURIComponent(options.url);
		var defaults = {
			btn : "",
			url : "",
			param : {},
		...
		var $success = function(result) {
			if($settings.url == '/real/markets.html') {
				//console.log(result);
			}
...
//					if ('?forwardUrl=/index.php?s=exc&c=userController&m=login' == window.location.search) {
//						//window.location.search = "";
//						//window.location.href = "/user/login.html";
//					} else {
//						window.location.href = "/index.php?s=exc&c=userController&m=login&forwardUrl=" + window.location.pathname + window.location.search;
//					}
					if (getQueryString('forwardUrl') == '/index.php?s=exc&c=userController&m=login') {
						//window.location.search = "";
	...
	//计时器
	counter:function (ele,second,tips){
